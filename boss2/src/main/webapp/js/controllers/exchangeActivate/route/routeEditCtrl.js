 ...
 * 核销渠道编辑
 */
angular.module('inspinia').controller('routeEditCtrl',function($scope,$http,i18nService,$state,$stateParams){
    ...
    $http.post("excActRoute/getRoute","id="+$stateParams.id,
    ...
    $scope.commitInfo=function () {
        ...
        $http.post("excActRoute/updateRoute","info="+angular.toJson($scope.addInfo),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
