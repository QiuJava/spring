 ...
 * 充值返活动详情
 */
angular.module('inspinia',['uiSwitch']).controller('diYongJinEditCtrl',function(i18nService,$scope,$http,$state,$stateParams,$compile,$filter,SweetAlert){

    //编辑券配置
    if("view" == $stateParams.view){
        $("#coupon_title").text("抵用金活动福利优惠券详情");
        $scope.isView = true;
    }else{
        $scope.isView = false;
        $("#coupon_title").text("抵用金活动福利优惠券修改");
    }
    if("add" != $stateParams.view){
        ...
        $scope.info={};
        $scope.couponEntity = {integralScale:100,cancelVerificationCode:2,couponCode:16,activetiyCode:16,isshelves:0,isstatus:0,couponType:3};
        $("#totalAmount").val(null);
        $scope.isView = false;
        $("#coupon_title").text("新增抵用金活动福利优惠券");
	}
