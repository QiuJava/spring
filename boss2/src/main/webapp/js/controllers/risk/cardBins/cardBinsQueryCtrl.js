...
angular.module('inspinia',['uiSwitch']).controller('cardBinsQueryCtrl',function($scope,$http,$state,$stateParams,$compile,$uibModal,SweetAlert,$log,i18nService,$document,$timeout){
    ...
    $scope.stateSelect=[{text:"全部",value:''},{text:"打开",value:1},{text:"关闭",value:0}];
    $scope.stateStr=angular.toJson($scope.stateSelect);

    $scope.cardTypeSelect=[{text:"全部",value:''},{text:"贷记卡",value:1},{text:"借记卡",value:2},{text:"其他",value:0}];
    ...
    $scope.clear=function(){
        $scope.info={cardNo:"",state:"",cardType:"",currency:"",cardBank:"",
    ...
    $scope.gridOptions={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'cardNo',displayName:'卡bin',width:180 },
            ...
            { field: 'id',displayName:'操作',pinnedRight:true,width:120,
                ...
                '<a ng-show="grid.appScope.hasPermit(\'cardBins.getCardBins\')" ui-sref="risk.editCardBins({id:row.entity.id})" target="_black" >修改</a>' +
    ...
    $scope.import=function(){
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href="cardBins/exportInfo?info="+encodeURI(angular.toJson($scope.info));
                }
            });
    };
