 ...
 * 产品编辑、详情
 */
angular.module('inspinia').controller('exchangeOrderDetailCtrl',function($scope,$http,i18nService,$stateParams){
    ...
    $http.post("exchangeOrder/getExchangeOrder","id="+$stateParams.id,
    ...
    $scope.getDataProcessing = function () {
        if($scope.dataSta){
            $http.get('exchangeOrder/getDataProcessing?id='+$stateParams.id)
                .success(function(data) {
                    if(data.status){
                        $scope.info.mobileUsername = data.order.mobileUsername;
                        $scope.info.businessCode = data.order.businessCode;
                        $scope.dataSta=false;
                    }else{
