...
angular.module('inspinia', ['infinity.angular-chosen', 'angularFileUpload']).controller('carOrderCtrl',function($scope,$http,i18nService,$document,SweetAlert,$timeout,FileUploader){
    ...
    $scope.accountStatusStr = '[{text:"已记账",value:"1"},{text:"未记账",value:"0"}]';
    ...
    $scope.resetForm();
    $scope.baseInfo = {
        createSTime:moment(new Date().getTime()).format('YYYY-MM-DD')+' 00:00:00',
        createETime:moment(new Date().getTime()).format('YYYY-MM-DD')+' 23:59:59',
        finishSTime:moment(new Date().getTime()).format('YYYY-MM-DD')+' 00:00:00',
        finishETime:moment(new Date().getTime()).format('YYYY-MM-DD')+' 23:59:59',
        violationSTime:moment(new Date().getTime()).format('YYYY-MM-DD')+' 00:00:00',
        violationETime:moment(new Date().getTime()).format('YYYY-MM-DD')+' 23:59:59'
    };
    //是否校验时间
    isVerifyTime = 1;//校验：1，不校验：0
    setBeginTime=function(setTime){
        $scope.baseInfo.createSTime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");
        $scope.baseInfo.finishSTime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");
        $scope.baseInfo.violationSTime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");

    }

    setEndTime=function(setTime){
        $scope.baseInfo.createETime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");
        $scope.baseInfo.finishETime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");
        $scope.baseInfo.violationETime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");
    ...
    $scope.columnDefs = [
        ...
        '<a class="lh30" rel="noopener noreferrer" target="_blank" ng-show="grid.appScope.hasPermit(\'superBank.carOrderDetail\')" '
