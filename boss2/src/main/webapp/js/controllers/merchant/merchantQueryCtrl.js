...
angular.module('inspinia', ['infinity.angular-chosen']).controller('merchantQueryCtrl',function($scope,$http,$state,$timeout,$stateParams,$compile,$filter,i18nService,SweetAlert,$document){
	...
	//数据源
	$scope.itemSource=[{text:"商户APP",value:"1"},{text:"代理商APP",value:"2"},{text:"代理商WEB",value:"3"},{text:"关联进件系统自动开通",value:"4"},{text:"还款系统",value:"5"}];
	...
	$scope.clear=function(){
		$scope.info={sTime:moment(new Date().getTime()-6*24*60*60*1000).format('YYYY-MM-DD')+' 00:00:00',
			eTime:moment(new Date().getTime()).format('YYYY-MM-DD'+' 23:59:59'),openStatus:"",
	...
	$scope.gridOptions={                           //配置表格
		...
		columnDefs:[                           //表格数据
			...
			{ field: 'id',displayName:'操作',width:270,pinnedRight:true,
				cellTemplate:
				'<div  class="lh30" ng-switch on="row.entity.status">'
				+'<div ng-switch-when="1">'
				+'<div ng-switch on="row.entity.merAccount">'
				+'<div ng-switch-when="1">'
				+ '<a ng-show="grid.appScope.hasPermit(\'merchant.detail\')" ui-sref="merchant.queryMerDetail({mertId:row.entity.id})" target="_black">详情</a><a ng-show="grid.appScope.hasPermit(\'merchant.restPwd\')" ng-click="grid.appScope.restPwd(row.entity.merchantNo,row.entity.teamId)"> | 密码重置</a>'
//				    					+'<a ng-show="grid.appScope.hasPermit(\'merchant.updateRiskStatus\')" ng-click="grid.appScope.upRiskStatusOpen(row.entity.merchantNo,row.entity.riskStatus)"> | 冻结状态</a>'
				+'<a ng-show="grid.appScope.hasPermit(\'merchant.preFrozen\')"  ng-click="grid.appScope.preFrozen(row.entity.merchantNo,row.entity.preFrozenAmount)"> | 预冻结</a>'
				+'</div>'
				+'<div ng-switch-when="0">'
				+ '<a ng-show="grid.appScope.hasPermit(\'merchant.detail\')" ui-sref="merchant.queryMerDetail({mertId:row.entity.id})" target="_black">详情</a><a ng-show="grid.appScope.hasPermit(\'merchant.openAccount\')" ng-click="grid.appScope.createAccount(row.entity.merchantNo)"> | 开户</a>'
...
//				    					+'<a ng-show="grid.appScope.hasPermit(\'merchant.updateRiskStatus\')" ng-click="grid.appScope.upRiskStatusOpen(row.entity.merchantNo,row.entity.riskStatus)"> | 冻结状态</a>'
				...
				+'<div ng-switch on="row.entity.merAccount">'
				+'<div ng-switch-when="1">'
				+ '<a ng-show="grid.appScope.hasPermit(\'merchant.detail\')" ui-sref="merchant.queryMerDetail({mertId:row.entity.id})" target="_black">详情</a><a ng-show="grid.appScope.hasPermit(\'merchant.update\')" ui-sref="merchant.MerUpdate({mertId:row.entity.id})" target="_black"> | 修改</a>'
				+'<a ng-show="grid.appScope.hasPermit(\'merchant.restPwd\')" ng-click="grid.appScope.restPwd(row.entity.merchantNo,row.entity.teamId)"> | 密码重置</a>'
//			    						+'<a ng-show="grid.appScope.hasPermit(\'merchant.updateRiskStatus\')" ng-click="grid.appScope.upRiskStatusOpen(row.entity.merchantNo,row.entity.riskStatus)"> | 冻结状态</a>'
				+'<a ng-show="grid.appScope.hasPermit(\'merchant.preFrozen\')"  ng-click="grid.appScope.preFrozen(row.entity.merchantNo,row.entity.preFrozenAmount)"> | 预冻结</a>'
				+'</div>'
				+'<div ng-switch-when="0">'
				+ '<a ng-show="grid.appScope.hasPermit(\'merchant.detail\')" ui-sref="merchant.queryMerDetail({mertId:row.entity.id})" target="_black">详情</a><a ng-show="grid.appScope.hasPermit(\'merchant.update\')" ui-sref="merchant.MerUpdate({mertId:row.entity.id})" target="_black"> | 修改</a>'
				+'<a ng-show="grid.appScope.hasPermit(\'merchant.openAccount\')" ng-click="grid.appScope.createAccount(row.entity.merchantNo)"> | 开户</a><a ng-show="grid.appScope.hasPermit(\'merchant.restPwd\')" ng-click="grid.appScope.restPwd(row.entity.merchantNo,row.entity.teamId)"> | 密码重置</a>'
//				    					+'<a ng-show="grid.appScope.hasPermit(\'merchant.updateRiskStatus\')"  ng-click="grid.appScope.upRiskStatusOpen(row.entity.merchantNo,row.entity.riskStatus)"> | 冻结状态</a>'
				+'<a ng-show="grid.appScope.hasPermit(\'merchant.preFrozen\')"  ng-click="grid.appScope.preFrozen(row.entity.merchantNo,row.entity.preFrozenAmount)"> | 预冻结</a>'
				+'</div>'
				+'</div>'
