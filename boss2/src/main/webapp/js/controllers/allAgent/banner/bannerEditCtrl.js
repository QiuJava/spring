 ...
 * banner新增
 */
angular.module('inspinia',['angularFileUpload']).controller('bannerAllAgentEditCtrl',function($scope,$http,i18nService,$state,FileUploader,$stateParams){
    ...
    uploaderImg.onSuccessItem = function(fileItem,response, status, headers) {// 上传成功后的回调函数，在里面执行提交
        if (response.url != null) { // 回调参数url
            $scope.saveBanner(response.url);
        }
    };

    $http.post("bannerAllAgent/getBanner",
        "id="+$stateParams.id,
        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
        .success(function(data){
            if(data.status){
    ...
    $scope.addBanner = function(){
        ...
        if(uploaderImg.queue.length<1){
            $scope.saveBanner(null);
        }else{
            uploaderImg.uploadAll();// 上传消息图片
        }
    };

    $scope.saveBanner=function (url) {
        ...
        $http.post("bannerAllAgent/updateBanner",
