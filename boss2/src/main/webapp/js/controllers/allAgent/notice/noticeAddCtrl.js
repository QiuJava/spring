 ...
 * 广告新增
 */
angular.module('inspinia',['angularFileUpload']).controller('noticeAllAgentAddCtrl',function($scope,$http,i18nService,$state,FileUploader){
    i18nService.setCurrentLang('zh-cn');  //设置语言为中文

    $scope.types= [{text:"普通公告",value:1},{text:"首页活动弹层",value:2}];
    $scope.addInfo={type:1};
    //组织列表
    ...
    $scope.addNotice = function(){
        ...
        $http.post("noticeAllAgent/addNotice",data,postCfg)
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
