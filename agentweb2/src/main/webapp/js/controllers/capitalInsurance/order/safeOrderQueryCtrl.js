 ...
 * 保险订单查询
 */
angular.module('inspinia').controller('safeOrderQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document){
    i18nService.setCurrentLang('zh-cn');  //设置语言为中文
    $scope.paginationOptions=angular.copy($scope.paginationOptions);

    $scope.bxUnitSelect =angular.copy($scope.insurerList);
    $scope.bxUnitSelect.unshift({text:"全部",value:""});
    ...
    $scope.clear=function(){
        $scope.info={bxUnit:"",bxType:"",transStatus:"",settlementMethod:"",oneAgentNo:"",prodNo:"",
            lowerAgent:0,sysSource:"",
            tTimeBegin:moment(new Date().getTime()).format('YYYY-MM-DD')+' 00:00:00',
    ...
    setBeginTime=function(setTime){
        $scope.info.tTimeBegin = moment(setTime).format("YYYY-MM-DD HH:mm:ss");

    }

    setEndTime=function(setTime){
        $scope.info.tTimeEnd = moment(setTime).format("YYYY-MM-DD HH:mm:ss");
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'nPrm',displayName:'保险订单金额',width:180,cellFilter:"currency:''" },
    ...
    $scope.query=function(){
        ...
        if(!( $scope.info.tTimeBegin &&  $scope.info.tTimeEnd)){
            $scope.notice("投保日期不能为空，请重新选择");
            return
        }
        $scope.loadImg = true;
    ...
    $scope.exportInfo = function () {
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href = "safeOrder/exportDetail?info=" + encodeURI(angular.toJson($scope.info));
                }
            });
    };
