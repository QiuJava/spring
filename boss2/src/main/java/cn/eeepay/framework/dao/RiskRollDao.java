...
public interface RiskRollDao {
	...
	@Insert("insert into risk_roll(roll_no,roll_name,roll_type,create_person,roll_belong,remark,roll_status,create_time)"
			+ " values(#{record.rollNo},#{record.rollName},#{record.rollType},#{record.createPerson},#{record.rollBelong},#{record.remark},#{record.rollStatus},NOW())")
	...
	@Update("update risk_roll set remark=#{record.remark} where id=#{record.id}")
	...
	RiskRoll checkRollByRollNo(@Param("rollNo")String rollNo,@Param("rollType") Integer rollType, @Param("rollBelong")Integer rollBelong);
	 ...
	 public class SqlProvider{
		 ...
		 public String selectRollAllInfo(Map<String,Object> param){
	    		...
	    		return new SQL(){{
	    			SELECT("rr.*,sd.sys_name as rollTypeName,bsu.user_name");
		 ...
		 public String selectRollAllInfoExport(Map<String,Object> param){
	    		final RiskRoll rr=(RiskRoll)param.get("record");
	    		return new SQL(){{
	    			SELECT("rr.*,sd.sys_name as rollTypeName");
	    			FROM("risk_roll rr "
