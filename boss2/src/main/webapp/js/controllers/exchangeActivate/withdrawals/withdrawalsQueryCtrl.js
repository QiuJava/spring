 ...
 * 用户提现记录查询
 */
angular.module('inspinia').controller('exchangeActivateWithdrawalsQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'userName',displayName:'昵称',width:180},
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("exchangeActivateWithdrawals/selectAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.exportInfo = function () {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href = "exchangeActivateWithdrawals/importDetail?info=" + encodeURI(angular.toJson($scope.info));
                }
            });
    };
    //页面绑定回车事件
