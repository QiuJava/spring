 ...
 * 积分兑换核销管理
 */
angular.module('inspinia',['angularFileUpload']).controller('exchangeAuditQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document,FileUploader){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'businessCode',displayName:'贡献人证件号',width:180 },
            ...
            { field: 'id',displayName:'操作',width:300,pinnedRight:true, cellTemplate:
                '<div class="lh30">'+
                '<a target="_blank" ui-sref="exchange.exchangeAuditDetail({id:row.entity.id})">详情</a> ' +
                '<a ng-show="row.entity.channelCheckStatus==0&&row.entity.checkStatusOne!=1&&row.entity.checkStatus==0&&row.entity.orderStatus==\'SUCCESS\'&&grid.appScope.hasPermit(\'exchangeOrder.orderApiSelect\')"  ng-click="grid.appScope.orderApiSelect(row.entity.id)"> | 核销查询</a> ' +
                '<a ng-show="(row.entity.channelCheckStatus==null||row.entity.channelCheckStatus==2)&&row.entity.checkStatusOne!=1&&row.entity.checkStatus==0&&row.entity.orderStatus==\'SUCCESS\'&&grid.appScope.hasPermit(\'exchangeOrder.orderApi\')"  ng-click="grid.appScope.orderApi(row.entity.id)"> | 通道核销</a> ' +
                '<a ng-show="(row.entity.channelCheckStatus==null||row.entity.channelCheckStatus==2)&&row.entity.checkStatusOne!=1&&row.entity.checkStatus==0&&row.entity.orderStatus==\'SUCCESS\'&&grid.appScope.hasPermit(\'exchangeOrder.updateWriteOff\')" target="_blank" ui-sref="exchange.exchangeAuditEdit({id:row.entity.id})"> | 核销</a> ' +
                '<a ng-show="(row.entity.channelCheckStatus==null||row.entity.channelCheckStatus==2)&&row.entity.checkStatusOne==1&&row.entity.checkStatus==0&&row.entity.orderStatus==\'SUCCESS\'&&grid.appScope.hasPermit(\'exchangeOrder.updateAgainWriteOff\')" target="_blank" ui-sref="exchange.exchangeAuditTwoEdit({id:row.entity.id})"> | 二次核销</a> ' +
                '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("exchangeOrder/selectAuditAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.exportInfo = function (sta) {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href="exchangeOrder/importAuditDetail?info="+encodeURI(angular.toJson($scope.info))+"&state="+sta;
                }
            });
    };
    /*
    ...
    $scope.orderApiSelect = function (id) {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("exchangeOrder/orderApiSelect","id="+id,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.orderApi = function (id) {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("exchangeOrder/orderApi","id="+id,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    var uploader = $scope.uploader = new FileUploader({
        url: 'exchangeOrder/importDiscount',
        queueLimit: 1,   //文件个数
