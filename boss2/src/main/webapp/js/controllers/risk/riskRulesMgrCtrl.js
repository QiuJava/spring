...
angular.module('inspinia',['uiSwitch']).controller('riskRulesMgrCtrl',function($scope, $http, $state, $stateParams, i18nService,$filter,SweetAlert,$document) {
	...
	$scope.rulesInstructions=[{text:"全部",value:-1},{text:"商户黑名单",value:1},{text:"身份证黑名单",value:2}
							,{text:"银行卡黑名单",value:3},{text:"钱包出账黑名单",value:4}
							,{text:"实名认证",value:5},{text:"商户号风险预警",value:6},{text:"银行卡号风险预警",value:7}
							,{text:"商户进件控制",value:8},{text:"商户资料提交控制",value:9}]
	...
	$scope.riskRulesGrid = {
	        ...
	        columnDefs: [
                ...
                {field: 'rulesInstruction',displayName: '规则指令',width: 120,pinnable: false,sortable: false,cellFilter:"rulesInstructionsNodes"},
	            ...
	            {field: 'rulesNo',displayName: '操作',width: 120,pinnedRight: true,sortable: false,editable:true,cellTemplate:
	            	'<div class="lh30">'
							...
							+"<a ng-show='grid.appScope.hasPermit(\"riskRule.update\") && row.entity.rulesNo!=124 " +
									"&& row.entity.rulesNo!=125 && row.entity.rulesNo!=126 && row.entity.rulesNo!=122 && row.entity.rulesNo!=123  && row.entity.rulesNo!=102 && row.entity.rulesNo!=103 && row.entity.rulesNo!=104 && row.entity.rulesNo!=105 && row.entity.rulesNo!=106 && row.entity.rulesNo!=107" +
                            		"&& row.entity.rulesNo!=108 && row.entity.rulesNo!=109 && row.entity.rulesNo!=110 && row.entity.rulesNo!=111 && row.entity.rulesNo!=112 && row.entity.rulesNo!=113 && row.entity.rulesNo!=114 && row.entity.rulesNo!=115" +
									"&& row.entity.rulesNo!=116 && row.entity.rulesNo!=117 && row.entity.rulesNo!=118" +
									"&& row.entity.rulesNo!=119 && row.entity.rulesNo!=120 && row.entity.rulesNo!=121' " +
...
.filter('rulesInstructionsNodes', function () {
	return function (value) {
		switch(value) {
			case 1 :
				return "商户黑名单";
