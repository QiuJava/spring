...
angular.module('inspinia',['angularFileUpload','uiSwitch']).controller('merchantUpdateCtrl',['$scope','$http','$filter','$log','$state','$stateParams','$compile','$uibModal','FileUploader',function($scope,$http,$filter,$log,$state,$stateParams,$compile,$uibModal,FileUploader){
	...
	$scope.terminal=[];
	var quotasbz=[];
	$http.get('merchantBusinessProduct/selectDetailInfoToUpdate.do?ids='+$stateParams.mertId)
	.success(function(largeLoad) {
		if(!largeLoad.bols){
	...
	$scope.merchantRateList={
		...
		columnDefs:[
		    ...
		    {field:'cardType',displayName:'银行卡种类',width:120,pinnable:false,sortable:false,
		    	cellFilter: "formatDropping:[{text:'不限',value:0},{text:'信用卡',value:1},{text:'储蓄卡',value:2}]"
		    },
		    {field:'holidaysMark',displayName:'节假日标志',width:80,pinnable:false,sortable:false,
		    	cellFilter: "formatDropping:[{text:'不限',value:0},{text:'只工作日',value:1},{text:'只节假日',value:2}]"
		    },
		    {field:'rateType',displayName:'费率方式',width:220,pinnable:false,sortable:false,
		    	cellFilter: "formatDropping:[{text:'每笔固定金额',value:'1'},{text:'每笔扣率',value:'2'},{text:'每笔扣率带保底封顶',value:'3'},{text:'每笔扣率+固定金额',value:'4'}"
						+",{text:'单笔阶梯扣率',value:'5'}]"
		    },
...
//		    }
			...
			for(var i=0;i<$scope.serviceStatus.length;i++){
				if($scope.serviceStatus[i].id==id){
					...
					.success(function(result){
						if(result.result){
							$scope.serviceStatus=result.serviceMgr;
							for(var i =0;i<$scope.serviceStatus.length;i++){
	...
	//审核审批
	$scope.requireItemstates=[{text:"待审核",value:0},{text:"审核通过",value:1},{text:"审核失败",value:2}];
	$scope.merchantDetailed={
			data:'listMri',
			columnDefs:[
			       {field:'itemName',displayName:'进件要求项名称'}, 
			       {field:'content',displayName:'资料',width:280,
			    	   cellTemplate:'<div ng-switch on="$eval(\'row.entity.exampleType\')">'
							...
							+'<div ng-switch-when="3">'
								+'<div ng-switch on="$eval(\'row.entity.status\')">'
									+'<div ng-switch-when="2">'
										+'<div ng-switch on="$eval(\'row.entity.mriId\')">'
											+'<div ng-switch-when="7">'
												+'<input style="width:250px; height: 30px" disabled="disabled" class="ui-widget input" ng-readonly="false" ng-model="row.entity[col.field]"/> <a ng-click="grid.appScope.updateAddress(row.entity[col.field],$eval(\'row.entity.mriId\'))">修改</a>'
											+'</div>'
											+'<div ng-switch-when="15">'
												+'<input style="width:250px; height: 30px" disabled="disabled" class="ui-widget input" ng-readonly="false" ng-model="row.entity[col.field]"/> <a ng-click="grid.appScope.updateAddress(row.entity[col.field],$eval(\'row.entity.mriId\'))">修改</a>'
											+'</div>'
											+'<div ng-switch-default>'
												+'<input style="width:250px; height: 30px" class="ui-widget input" ng-readonly="false" ng-model="row.entity[col.field]"/>'
											+'</div>'
										+'</div>'
									+'</div>'
									+'<div ng-switch-default>'
										+'<input style="width:250px; height: 30px" disabled="disabled" class="ui-widget input" ng-readonly="false" ng-model="row.entity[col.field]"/>'
									+'</div>'
								+'</div>'
							+'</div>'
							+'</div>'
	...
	$scope.commitInfo=function(){
		...
		if(fls==0){
			var data={"mbp":$scope.mbp,"info":$scope.info,"listMri":$scope.listMri,"listMsr":$scope.rates,"listMsq":$scope.quotas,"status":2};
	...
	$scope.commit=function(){
		$scope.submitting = true;
		if(fls==0){
			var data={"mbp":$scope.mbp,"info":$scope.info,"listMri":$scope.listMri,"listMsr":$scope.rates,"listMsq":$scope.quotas,"status":0};
			$http.post("merchantBusinessProduct/updateMerchantInfo.do",angular.toJson(data))
			.success(function(result){
				if(result.result){
					$scope.notice(result.msg);
	...
	//修改进件资料
	$scope.updateMriInfo=function(mriId,val,id,merBpId,status,content,name){
			$scope.submitting = true;
