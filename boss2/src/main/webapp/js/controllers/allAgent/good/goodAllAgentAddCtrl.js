 ...
 * 商品新增
 */
angular.module('inspinia',['angularFileUpload']).controller('goodAllAgentAddCtrl',function($scope,$http,i18nService,FileUploader,$stateParams,$state){
    i18nService.setCurrentLang('zh-cn');  //设置语言为中文
    $scope.addInfo={brandCode:"",isMulti:0,shipWay:""};
    ...
    $scope.saveProduct=function () {
        ...
        var groupCode = $scope.addInfo.groupCode;
        if (groupCode === "" || typeof(groupCode) === "undefined") {
        ...
        if($scope.uploaderImg1.queue.length<1){
            $scope.notice("商品主图1不能为空!");
            return;
        }
        $scope.submitting = true;
        $scope.subAddinfo=angular.copy($scope.addInfo);
        $scope.subAddinfo.goodsDesc=null;
        var data={
        ...
        $http.post("goodAllAgent/addGood",data,postCfg)
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
    ...
    //     if (files != null) {
    //
    //         var uploaderNoteImg= $scope.uploaderNoteImg  = new FileUploader({
    //             url: 'upload/fileUploadLongTime.do',
    //             queueLimit: 1,   //文件个数
    //             removeAfterUpload: false,  //上传后删除文件
    //             autoUpload:true,     //文件加入队列之后自动上传，默认是false
    //             headers : {'X-CSRF-TOKEN' : $scope.csrfData}
    //         });
    //         uploaderNoteImg.onSuccessItem = function(fileItem,response, status, headers) {// 上传成功后的回调函数，在里面执行提交
    //             if (response.url != null) { // 回调参数url
    //                 $("#summernote").summernote("editor.insertImage",response.url);
    //             }
    //         };
    //         uploaderNoteImg.addToQueue(files[0]);
    //     }
    // };
