 ...
 * 激活码查询
 */
angular.module('inspinia').controller('activationCodeCtrl',function($scope,$http,$state,$stateParams,i18nService){
	//数据源
    ...
    $scope.info  = {
        ...
        startActivateTime:moment(new Date().getTime()).format('YYYY-MM-DD'+' 00:00:00'),
        endActivateTime:moment(new Date().getTime()).format('YYYY-MM-DD')+' 23:59:59'
    };
    //是否校验时间
    isVerifyTime = 1;//校验：1，不校验：0
    setBeginTime=function(setTime){
        $scope.info.startActivateTime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");

    }

    setEndTime=function(setTime){
        $scope.info.endActivateTime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");
	...
	$scope.listActivationCode = function () {
        if (!( $scope.info.startActivateTime &&  $scope.info.endActivateTime)){
    ...
    $http.post("agentInfo/selectSelfAndDirectChildren")
        .success(function(msg){
            for(var i=0; i<msg.length; i++){
                ...
                $scope.agentChildrenList.push({value:msg[i].agentNo,text:msg[i].agentName});
            }
        });
    $http({
