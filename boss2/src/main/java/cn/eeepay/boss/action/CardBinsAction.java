...
public class CardBinsAction {
    ...
    public Map<String, Object> saveCardBins(@RequestParam("info") String param) throws Exception {
        ...
        try {
            ...
            int ret = 0;
            if(card.getId()==null){
                CardBins oldCard =cardBinsService.getCardBinsByCardNo(card.getCardNo());
                if(oldCard!=null){
                    msg.put("status", false);
            ...
            log.error("保存卡bin信息失败!",e);
            msg.put("status", false);
            msg.put("msg", "保存卡bin信息失败!");
        }
        return msg;
    }
    ...
    @ResponseBody
    public Map<String, Object> importCardBins(@RequestParam("file") MultipartFile file){
        Map<String, Object> msg = new HashMap<>();
        try {
            if (!file.isEmpty()) {
            ...
            msg = cardBinsService.importCardBins(file);
        } catch (Exception e) {
            msg.put("status", false);
            msg.put("msg", "导入失败");
            log.error("导入失败",e);
