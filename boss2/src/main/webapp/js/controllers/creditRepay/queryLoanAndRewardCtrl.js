...
angular.module('inspinia', ['infinity.angular-chosen']).controller('queryLoanAndRewardCtrl',function($scope,$http,$state,$timeout,$stateParams,$compile,$filter,i18nService,SweetAlert,$document){
	...
	//获取所有赠送类型
	$scope.sendTypeList=[];
	$http.get('cardAndReward/getSendTypeList?type=13')
		.success(function(data){
			  //响应成功
	...
	$scope.gridOptions={                           //配置表格
		...
		columnDefs:[                           //表格数据
			...
			{ field: 'id',displayName:'操作',width:270,pinnedRight:true,
				cellTemplate:
					'<a class="lh30" ng-show="grid.appScope.hasPermit(\'loanAndReward.send\')&&row.entity.givenStatus!=1" ng-click="grid.appScope.hardWardAddModel(row.entity.id,1)">赠送  | </a>'
    ...
    $scope.addAgentAccountControl = function(){
		...
		$http.post('cardAndReward/allSend',
				"baseInfo="+angular.toJson(merchantNoList)+"&info="+angular.toJson($scope.info)+"&type="+13,{headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
				.success(function(data){
					if(data.bols){
