...
public interface LotteryOrderDao {
	 ...
	 class SqlProvider{
		 public String selectLotteryOrder(Map<String,Object> param){
			 ...
			 sql.SELECT("ly.order_no,ly.device_jno,oi.org_id,oi.org_name,om.status order_status,ui.user_code,ui.user_name,ui.phone,om.price,om.loan_amount,om.profit_type,ly.award_require,om.loan_bank_rate,om.total_bonus,om.loan_org_rate,"+
						...
						"ui.open_province,ui.open_city,ui.open_region,om.remark,om.company_bonus_conf,om.org_bonus_conf,ly.buy_status,ly.menchant_no,om.create_date ");
			 whereSql(sql,info);
		 ...
		 public String selectOrderSum(Map<String,Object> param){
			 final LotteryOrder info = (LotteryOrder)param.get("baseInfo");
			 SQL sql = new SQL();
			 sql.SELECT("sum(price)-sum(company_bonus) as sumAmount");
	...
	@Insert("insert into lottery_order " +
			"(out_id,order_no,out_order_no,out_user_id,device_no,device_jno,menchant_no,menchant_name,lottery_type,issue,bet_time,redeem_flag,redeem_time,buy_status,is_big_prize,consume_e,award_amount,create_date,remark,user_code,award_require,batch_no)" +
			" values " +
			"(#{outId},#{orderNo},#{outOrderNo},#{outUserId},#{deviceNo},#{deviceJno},#{menchantNo},#{menchantName},#{lotteryType},#{issue},#{betTime},#{redeemFlag},#{redeemTime},#{buyStatus},#{isBigPrize},#{consumeE},#{awardAmount},sysdate(),'100e豆=1元',#{userCode},#{awardRequire},#{batchNo})")
