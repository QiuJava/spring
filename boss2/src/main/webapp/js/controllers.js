...
function MainCtrl($scope,notify,$http) {
	...
	$scope.holidays=[{text:"不限",value:0},{text:"只工作日",value:1},{text:"只节假日",value:2}];
	$scope.holidaysStr='[{text:"不限",value:0},{text:"只工作日",value:1},{text:"只节假日",value:2}]';
	$scope.fixedMark=[{text:"固定",value:1},{text:"不固定",value:0}];
	$scope.accountType=[{text:"对公",value:1},{text:"对私",value:2}];
	$scope.teamType=[{text:"直营组织",value:1},{text:"虚拟组织",value:2}];
	$scope.checkType=[{text:"已审核",value:1},{text:"未审核",value:0}];
	$scope.lockType=[{text:"锁定",value:1},{text:"未锁定",value:0}];
	$scope.status=[{text:"有效",value:1},{text:"无效",value:0}];
	$scope.statusAll=[{text:"全部",value:2},{text:"开启",value:1},{text:"关闭",value:0}];
	$scope.merStatusList=[{text:"关闭",value:"0"},{text:"正常",value:"1"},{text:"冻结",value:"2"}];
	$scope.merchantTypes = [{text:"个人",value:1},{text:"个体商户",value:2},{text:"企业商户",value:3}];
	$scope.cancelVerType = [{text:"全部",value:''},{text:'扣减交易手续费',value:'2'},{text:'扣减提现手续费',value:'1'},{text:'过期',value:'3'},{text:'抵扣自选商户手续费',value:'4'}];
	$scope.couponStatus = [{text:'使用部分',value:'2'},{text:'已使用',value:'3'},{text:'未使用',value:'1'},{text:'已过期',value:'4'}];

	//活动状态
	$scope.activityStatus = initData.ACTIVITY_STATUS;
	$scope.activityStatusAll = angular.copy(initData.ACTIVITY_STATUS);	
	$scope.activityStatusAll.unshift({text:"全部",value:""});
	
	//机具通道
	//$scope.JJTD = initData.JJTD;
	//$scope.JJTDAll = angular.copy(initData.JJTD);	
	//$scope.JJTDAll.unshift({text:"全部",value:""});
	
	//核算状态
	$scope.checkStatus = initData.CHECK_STATUS;
	$scope.checkStatusAll = angular.copy(initData.CHECK_STATUS);	
	$scope.checkStatusAll.unshift({text:"全部",value:""});
	// 产品服务类型 begin
	$scope.serviceTypes = initData.SERVICE_TYPE;	//服务类型
	$scope.serviceTypeAll = angular.copy(initData.SERVICE_TYPE);	
	$scope.serviceTypeAll.unshift({text:"全部",value:""});
	
	$scope.exampleType = initData.ITEM_TYPE;		//进件要求项类型
	
	//新增业务产品时，预选中的进件要求项
	$scope.SELECTED_ITEMS = initData.SELECTED_ITEMS;
	
	//账户科目
	$scope.accountSubjectList = initData.ACCOUNT_SUBJECT_NO;
	$scope.accountSubjectListAll = angular.copy(initData.ACCOUNT_SUBJECT_NO);	
	$scope.accountSubjectListAll.unshift({text:"请选择",value:""});
	
	//收款码管理
	$scope.materialType = initData.MATERIAL_TYPE;
	$scope.materialTypeAll = angular.copy($scope.materialType);
	$scope.materialTypeAll.unshift({text:"全部",value:-1});
	
	$scope.blacklistTypes = initData.ROLL_TYPE;		//黑名单、白名单：类型
	$scope.blacklistTypesAll = angular.copy(initData.ROLL_TYPE);
	$scope.blacklistTypesAll.unshift({text:"全部",value:-1});
	//白名单类型
	$scope.whitelistTypes = initData.WHITE_ROLL_TYPE;
	$scope.whitelistTypesAll = angular.copy(initData.WHITE_ROLL_TYPE);
	$scope.whitelistTypesAll.unshift({text:"全部",value:-1});
	//预警方式
	$scope.warns = initData.RULES_WARN;
	$scope.warnsAll = initData.RULES_WARN;
	$scope.warnsAll.unshift({text:"请选择",value:""});
	//生效节点
	$scope.nodes = initData.RULES_NODE;
	$scope.nodesAll = initData.RULES_NODE;
	$scope.nodesAll.unshift({text:"请选择",value:""});
	// 收单服务类型  begin
	$scope.acqServiceTypes = initData.ACQ_SERVICE_TYPE;
	$scope.acqServiceTypeAll = angular.copy(initData.ACQ_SERVICE_TYPE);
	$scope.acqServiceTypeAll.unshift({text:"全部",value:""});
	
	//出款服务类型
	$scope.moneyServiceType = initData.MONEY_SERVICE_TYPE;
	$scope.moneyServiceTypeAll = angular.copy(initData.MONEY_SERVICE_TYPE);
	$scope.moneyServiceTypeAll.unshift({text:"全部",value:""});
	
	//出款提现类型
	$scope.outSubType = initData.OUT_SUB_TYPE;
	$scope.outSubTypeAll = angular.copy(initData.OUT_SUB_TYPE);
	$scope.outSubTypeAll.unshift({text:"全部",value:-1});
	
	$scope.rey = initData.ACQ_BILL_TYPE;
	
	$scope.transStatus = initData.TRANS_STATUS;	//交易状态
	$scope.transStatusAll = angular.copy(initData.TRANS_STATUS);	
	$scope.transStatusAll.unshift({text:"全部",value:""});
	$scope.transStatusStr = angular.toJson($scope.transStatus);
	
	$scope.transTypes = initData.TRANS_TYPE;	//交易类型
	
	$scope.payMethodType = initData.PAY_METHOD_TYPE;	//交易方式
	$scope.payMethodTypeAll = angular.copy(initData.PAY_METHOD_TYPE);	
	$scope.payMethodTypeAll.unshift({text:"全部",value:""});
	
	$scope.settleStatus = initData.SETTLE_STATUS;	//结算方式
	$scope.settleStatusAll = angular.copy(initData.SETTLE_STATUS);	
	$scope.settleStatusAll.unshift({text:"全部",value:""});
	$scope.settleStatusStr = angular.toJson($scope.settleStatus);
	
	$scope.outStatus = initData.OUT_STATUS;	//出款状态
	$scope.outStatusAll = angular.copy(initData.OUT_STATUS);
	$scope.outStatusAll.unshift({text:"全部",value:""});
	$scope.outStatusStr = angular.toJson($scope.outStatus);

	$scope.settleType = initData.SETTLE_TYPE;	//结算类型
	$scope.settleTypeAll = angular.copy(initData.SETTLE_TYPE);	
	$scope.settleTypeAll.unshift({text:"全部",value:""});
	$scope.settleTypeStr = angular.toJson($scope.settleType);
	
	$scope.cardType = initData.CARD_TYPE;	//卡类型
	$scope.cardTypeAll = angular.copy(initData.CARD_TYPE);	
	$scope.cardTypeAll.unshift({text:"全部",value:""});
	$scope.cardTypeStr = angular.toJson($scope.cardType);
	//代理商层级长度
	$scope.agentLevel = initData.AGENT_LEVEL;
//	$scope.agentLevelAll = angular.copy($scope.agentLevel);
//	$scope.agentLevelAll.unshift({text:"全部",value:""});
	//收单商户类别
	$scope.acqMerchantTypes=initData.ACQ_MERCHANT_TYPE;
	$scope.acqMerchantTypesAll = angular.copy(initData.ACQ_MERCHANT_TYPE);	
	$scope.acqMerchantTypesAll.unshift({text:"全部",value:-1});
	
	//banner位置
	$scope.bannerPosition = initData.BANNER_POSITION;
	$scope.bannerPositionAll = angular.copy(initData.BANNER_POSITION);	
	$scope.bannerPositionAll.unshift({text:"全部",value:-1});
	
	//服务模式
	$scope.serviceModels = initData.SERVICE_MODEL;
	
	//直清通道
	$scope.liquidationChannels = initData.LIQ_CHANNEL;
	
	//机具活动类型
	$scope.activityTypeList = initData.ACTIVITY_TYPE;
	$scope.activityTypeListAll = angular.copy(initData.ACTIVITY_TYPE);	
	$scope.activityTypeListAll.unshift({text:"全部",value:""});
	$scope.activityTypeListNull = angular.copy(initData.ACTIVITY_TYPE);
	$scope.activityTypeListNull.unshift({text:"无",value:""});
	
	//优惠券活动类型
	$scope.couponCode = initData.COUPON_CODE;
	$scope.couponCodes = angular.copy(initData.COUPON_CODE);	
	$scope.couponCodes.unshift({text:"全部",value:""});
	
	//订单类型
	$scope.orderType = initData.ORDER_TYPE;
	$scope.orderTypeAll = angular.copy(initData.ORDER_TYPE);	
	$scope.orderTypeAll.unshift({text:"全部",value:""});

	//超级银行家UI类型
    $scope.uiStatusList = initData.UI_STATUS_LIST;

    //来源系统
    $scope.sourceSystem = initData.SOURCE_SYSTEM;
    $scope.sourceSystems = angular.copy(initData.SOURCE_SYSTEM);
    $scope.sourceSystems.unshift({text:"全部",value:""});

    //超级银行家红包业务类型
    $scope.redBusTypeList = initData.RED_BUS_TYPE;
    $scope.redBusTypeListAll = angular.copy(initData.RED_BUS_TYPE);
    $scope.redBusTypeListAll.unshift({text:"全部",value:""});

    //红包业务类型
    $scope.redBusType = initData.RED_BUS_TYPE;
    $scope.redBusTypes = angular.copy(initData.RED_BUS_TYPE);
    $scope.redBusTypes.unshift({text:"全部",value:""});


    //发放人类型
    $scope.redPushType = initData.RED_PUSH_TYPE;
    $scope.redPushTypes = angular.copy(initData.RED_PUSH_TYPE);
    $scope.redPushTypes.unshift({text:"全部",value:""});

    //接收类型 receive_type
    $scope.redReceiveType = initData.RED_RECEIVE_TYPE;
    $scope.redReceiveTypes = angular.copy(initData.RED_RECEIVE_TYPE);
    $scope.redReceiveTypes.unshift({text:"全部",value:""});

    //接收地区 push_area
    $scope.redPushArea = initData.RED_PUSH_AREA;
    $scope.redPushAreas = angular.copy(initData.RED_PUSH_AREA);

    $scope.redPushAreas.unshift({text:"全部",value:""});

    //红包配置状态 push_area
    $scope.redStatus = initData.RED_STATUS;
    $scope.redStatusList = angular.copy(initData.RED_STATUS);
    $scope.redStatusList.unshift({text:"全部",value:""});

    //卡bin交易币种
    $scope.cardBinsCurrency = initData.cardBinsCurrency;
    $scope.cardBinsCurrencys = angular.copy(initData.cardBinsCurrency);
    $scope.cardBinsCurrencys.unshift({text:"全部",value:""});
    $scope.cardBinsCurrencys.push({text:"其他",value:0});

    $scope.cardBinsCurrencysAdd = angular.copy(initData.cardBinsCurrency);
    $scope.cardBinsCurrencysAdd.unshift({text:"请选择",value:null});
    $scope.cardBinsCurrencysAdd.push({text:"其他",value:0});

    //超级兑用户身份
    $scope.merCapaExchange = angular.copy(initData.MER_CAPA);
    $scope.merCapaExchange.unshift({text:"全部",value:""});

    //超级兑订单状态
    $scope.orderStatusExchange = angular.copy(initData.ORDER_STATUS);
    $scope.orderStatusExchange.unshift({text:"全部",value:""});

    //超级兑记账状态
    $scope.accStatusExchange = angular.copy(initData.ACC_STATUS);
    $scope.accStatusExchange.unshift({text:"全部",value:""});

    //超级兑核销渠道
    $scope.orderChannelExchange= angular.copy(initData.ORDER_CHANNEL);

    //超级盟主,代理商组织
    $scope.allAgentOem = angular.copy(initData.AGENT_OEM);
    $scope.allAgentOemText=angular.copy(initData.AGENT_OEM);
    $scope.allAgentOem.unshift({text:"全部",value:""});
    //代理商类型
    $scope.allAgentType = angular.copy(initData.AGENT_TYPE);
    $scope.allAgentTypeText=angular.copy(initData.AGENT_TYPE);
    $scope.allAgentType.unshift({text:"请选择",value:""});
    $scope.allAgentExpress = angular.copy(initData.AGENT_EXPRESS);
    $scope.allAgentExpress.unshift({text:"请选择",value:""});
    //超级盟主售后类型
    $scope.saleTypes = angular.copy(initData.RRDL_SALE_TYPE);
    $scope.saleTypes.unshift({text:"全部",value:""});
    //超级盟主收支类别
    $scope.transTypes = angular.copy(initData.INCOME_TYPE);
    $scope.transTypes.unshift({text:"全部",value:""});
    ...
    //评论删除原因
    $scope.discussDeleteReasonList = initData.DISCUSS_DELETE_REASON;
    //风控关闭原因
    $scope.riskCloseReasonLost = initData.RISK_CLOSE_REASON;
    //红包配置状态 push_area
    $scope.redTypeList = initData.RED_TYPE;
    $scope.redTypeListAll = angular.copy(initData.RED_TYPE);
    $scope.redTypeListAll.unshift({text:"全部",value:""});

    //首页样式
    $scope.indexStyleList = initData.INDEX_STYLE;

    //资金损失险
    //承保单位
    $scope.insurerList= angular.copy(initData.insurer_number);
    $scope.insurerList.unshift({text:"全部",value:"0"});


    //调单管理
    //调单类型
    $scope.orderTypeCodeList= angular.copy(initData.ORDER_TYPE_CODE);
    $scope.orderTypeCodeList.unshift({text:"全部",value:""});
    //业务类型
    $scope.orderServiceCodeList= angular.copy(initData.ORDER_SERVICE_CODE);
    $scope.orderServiceCodeList.unshift({text:"全部",value:""});
    //回复状态
    $scope.orderReplyResultList= angular.copy(initData.ORDER_REPLY_RESULT);
    $scope.orderReplyResultList.unshift({text:"全部",value:""});

    //商户进件来源
    $scope.merItemSourceList= angular.copy(initData.MER_ITEM_SOURCE);
    $scope.merItemSourceList.unshift({text:"全部",value:""});

    //奖券用途
    $scope.cancelVerificationList= angular.copy(initData.CANCEL_VERIFICATION_CODE);

    //商户类型
    $scope.merchantTypeLists= angular.copy(initData.MERCHANT_TYPE_LIST);
    $scope.merchantTypeLists.unshift({text:"全部",value:""});
    //机具PN型号
    $scope.devicePNTypeLists= angular.copy(initData.DEVICE_TYPE_LIST);
    $scope.devicePNTypeLists.unshift({text:"无",value:null});

    //收单商户来源途径
    $scope.acqMerSourceList = angular.copy(initData.acqMerSource);

    //超级推支付通道
    $scope.cjtAcqCodeList = initData.CJT_ACQ_CODE_LIST;
    if($scope.cjtAcqCodeList != null && $scope.cjtAcqCodeList != ""
        && $scope.cjtAcqCodeList != "undefined"){
        $scope.cjtAcqCodeListAll = angular.copy($scope.cjtAcqCodeList);
        $scope.cjtAcqCodeListAll.unshift({text:"全部",value:""});
    }

    //超级推售后类型
    $scope.afterSaleTypeList = initData.AFTER_SALE_TYPE;
    if($scope.afterSaleTypeList != null && $scope.afterSaleTypeList != ""
        && $scope.afterSaleTypeList != "undefined"){
        $scope.afterSaleTypeListAll = angular.copy($scope.afterSaleTypeList);
        $scope.afterSaleTypeListAll.unshift({text:"全部",value:null});
    }

    //商户的推广来源
    $scope.recommendedSources = initData.RECOMMENDED_SOURCES;
    if($scope.recommendedSources != null && $scope.recommendedSources != ""
        && $scope.recommendedSources != "undefined"){
        $scope.recommendedSources.unshift({text:"全部",value:""});
    }
     ...
     * 富文本上传
     */
    $scope.imageUploadALL = function(files){
        if (files != null&&files.length>0) {
            for(var i=0;i<files.length;i++){
                var formData = new FormData();
                formData.append('file', files[i]);
                $http.post('upload/fileUploadLongTime.do',formData,
                    {headers: {'Content-Type':undefined}})
                    .success(function(req){
                        if (req.url != null) { // 回调参数url
                            $("#summernote").summernote("editor.insertImage",req.url);
                        }
                    }).error(function(){
                });
            }
     ...
     */
    $scope.exportInfoClick=function (url,params) {
        $http({
            url: url,
            method: "POST",
 ...
 * isVerifyTime: 是否校验时间，校验：1，不校验：0
 */
verifyTime2=function(type,st,et,day,isVerifyTime){
    if (!isVerifyTime) {
        return;
    }
    var stime = new Date($(st).val()).getTime();
