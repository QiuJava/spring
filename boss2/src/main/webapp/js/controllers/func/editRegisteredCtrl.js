 ...
 * 注册返编辑
 */
angular.module('inspinia').controller('editRegisteredCtrl',function(SweetAlert,$scope,$http,$stateParams,$state){
	...
	$scope.myGrid = {                  //配置表格
	    ...
	    columnDefs:[                        //表格数据
			...
			{ field: 'action',displayName:'操作',width:200,
				cellTemplate:
	      		'<a class="lh30" ng-show="row.entity.action==1 && grid.appScope.hasPermit(\'func.editcouponActivity\')" ng-click="grid.appScope.actEdit(row.entity)"><input type="hidden" ng-model="row.entity.action" />编辑</a>'+
				'<a class="lh30" ng-show="row.entity.action==2 && grid.appScope.hasPermit(\'func.editcouponActivity\')" ng-click="grid.appScope.actSave(row.entity)">保存</a>'
			}
	    ]
	};
	...
	$scope.commit = function(){
		...
		$http.post('couponActivity/registeredSave',
