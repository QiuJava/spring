...
angular.module('inspinia',['angularFileUpload','infinity.angular-chosen']).controller('happyBackCtrl',function($scope,$http,$state,$stateParams,i18nService,$document,SweetAlert,FileUploader,$timeout){
	...
	$scope.resetForm=function(){
		$scope.baseInfo = {merchantN:"",agentNode:"",status:"",discountStatus:-1,checkStatus:"",oneAgentNo:"",
			...
			minAddAmountTime:'', maxAddAmountTime:'',acqOrgId:'',countAll:true,pageAll:false,checkIds:null};
    ...
    $scope.countTradeColumnDefs = [
       /* {field: 'id',displayName: 'ID',pinnable: false,width: 50,sortable: false},*/
    ...
    $scope.payMethodCheck= function(){
        ...
        $http.get("activity/setCountTradeScope?countTradeScope="+ angular.toJson($scope.checkMap))
	...
	$scope.activityGrid = {
		...
		columnDefs: [
			...
			{field: 'action',displayName: '操作',width: 250,pinnedRight:true,sortable: false,editable:true,cellTemplate:
					...
					// +'<a class="lh30" ng-click="grid.appScope.accountCheck(row.entity)"'
					// +' ng-show="row.entity.activityCode==\'009\' && row.entity.status==2 && row.entity.liquidationStatus==1 '
					// +' && row.entity.accountCheckStatus!=1 && grid.appScope.hasPermit(\'activity.accountCheck\') ">财务核算</a>'
