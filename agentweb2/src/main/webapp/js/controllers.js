 ...
 *
 */
function MainCtrl($scope,$rootScope,$http,notify, $cookies,$uibModal,$httpParamSerializerJQLike, $templateCache) {
	$rootScope.username = _username;
	$rootScope.userid = _userid;
	$rootScope.teamId = _teamId;
	$rootScope.entityId = _entityId;
	$rootScope.entityName = _entityName;
	$rootScope.entityAgentLevel = _Dlevel;
	$rootScope.isAgent = _isAgent;
	$rootScope.permits = _permits;
	$rootScope.oemType = _oemType;
	$rootScope.agentType = _agentType;
	$rootScope.oemTransType = _oemTransType;
	$rootScope.loginUserType = _loginUserType;
	$rootScope.registType = _registType;
	$rootScope.hasSafePhone = _hasSafePhone;
	$rootScope.hasSafePassword = _hasSafePassword;
	$rootScope.safePasswordFlag = _safePasswordFlag;
	$rootScope.hasPermit = function(permit){
		return $rootScope.permits.indexOf(permit) >= 0;
	};
	$templateCache.put('__ajaxLogin.html', '<div class="ajaxloginblock"><iframe onload="__loginIframeOnload(this)" style="width:100%;height:100%" src="oemlogin.do?logout"></iframe></div>');
	var uibModalInstanceLogin;
	$rootScope.$on('responseError401', function(event, response, defer, headers, csrfHeadName){
		window.__loginIframeOnload = function(item){
			var that = $(item);
			var form = $(that.get(0).contentDocument).find('form');
			form.submit(function(){
				var param = {};
				form.find('[name]').map(function(){
					param[$(this).attr('name')] = $(this).val();
				});
				$http({
					url:'perform_login.do',
					method: 'post',
					...
					if(data.status){
						...
						if($rootScope.userid != userid || $rootScope.entityId != entityId){
							alert('你使用了不同的账号登录，将重新加载本页面');
							location.reload();
						}
						$rootScope.username = data.username;
						$rootScope.userid = data.userid;
						$rootScope.teamId = data.teamId;
						$rootScope.entityId = data.entityId;
						$rootScope.entityName = data.entityName;
						$rootScope.permits = data.permits;
                        $rootScope.oemType = data.oemType;
						delete headers[csrfHeadName];
						csrfHeadName = data._csrf.headerName;
						headers[csrfHeadName] = data._csrf.token;
						uibModalInstanceLogin.close();
					}else{
						$rootScope.notice(data.message || '登录失败');
					}
				});
				return false;
			})
		...
		delete response.config.headers[csrfHeadName];
		if(uibModalInstanceLogin){
			// 已经打开了，等待登录窗口完成
			uibModalInstanceLogin.result.then(function(){
		...
		uibModalInstanceLogin = $uibModal.open({
			controller:function($scope){
			},
			show: true,
			templateUrl: '__ajaxLogin.html'
		});
		uibModalInstanceLogin.result.then(function(){
			$http(response.config).then(function(data){
				defer.resolve(data);
		...
		uibModalInstanceLogin.result.then(function(){
			uibModalInstanceLogin = null;
		},function(){
			uibModalInstanceLogin = null;
		});
	});
	$rootScope.modifyPasswordShow = function(){
		...
		var uibModalInstance = $uibModal.open({
			controller:function($scope){
				$scope.submitNewUserPwd = function(){
					...
					$http.post('userAction/updatePwd.do',angular.toJson(data))
	...
	$scope.holidaysStr='[{text:"不限",value:0},{text:"只工作日",value:1},{text:"只节假日",value:2},{text:"全部",value:3}]';
	...
	$scope.statusAll=[{text:"全部",value:2},{text:"有效",value:1},{text:"无效",value:0}];
	$scope.userTypes=[{text:"全部",value:""},{text:'普通用户',value:10},{text:'专员',value:20},{text:'经理',value:30},{text:'银行家',value:40}];

	// 默认活动选项配置
    $scope.activityTypess=activitys;
    $scope.subjectNoss=subjectNos;//账户类型tgh330
    // initData.ORDER_TYPE = [
    //     {"text": "普通订单","value": 0},
    //     {"text": "微创业订单","value": 1},
    //     {"text": "欢乐送","value": 2},
    ...
    //     {"text": "境外卡交易","value": 7}
    // ];
	if($scope.oemType === "ZHFPAY"){			// 中和付
        $scope.activityTypess=[
            {sys_value: '7',sys_name:"欢乐返"},
            {sys_value: '8',sys_name:"欢乐返-循环送"},
            {sys_value: '11',sys_name:"注册返"}
        ];
        initData.ORDER_TYPE = [
            {"text": "普通订单","value": 0},
            {"text": "微创业订单","value": 1},
	...
	}else if($scope.oemType === "ZHBPAY"){			// 中和宝
        $scope.activityTypess=[
            {sys_value: '7',sys_name:"欢乐返"},
            {sys_value: '8',sys_name:"欢乐返-循环送"},
            {sys_value: '11',sys_name:"注册返"}
	...
	}else if ($scope.oemType === "YPOSPAY") {	// 银pos 银惠宝
        $scope.activityTypess=[
            {sys_value: '7',sys_name:"欢乐返"},
            {sys_value: '8',sys_name:"欢乐返-循环送"},
            {sys_value: '11',sys_name:"注册返"}
	...
	}else if ($scope.oemType === "YLSTCZB"){	// 移联商通成长版
        $scope.activityTypess=[
            {sys_value: '7',sys_name:"欢乐返"},
            {sys_value: '8',sys_name:"欢乐返-循环送"},
            {sys_value: '11',sys_name:"注册返"},
            {sys_value: '12',sys_name:"注册返300"}
        ];
        initData.ORDER_TYPE = [
            {"text": "普通订单","value": 0},
            {"text": "欢乐返","value": 3}
        ];
	}else if($scope.oemType === "ZHZFPAY"){			// 中和支付
        $scope.activityTypess=[
            {sys_value: '7',sys_name:"欢乐返"},
            // {sys_value: '8',sys_name:"欢乐返-循环送"},
            {sys_value: '12',sys_name:"注册返300"}
        ...
        $scope.subjectNoss = [
            {sys_name: "代理商分润", sys_value: "224105"},
			{sys_name: "活动补贴账户", sys_value: "224106"}
			// {sys_name: "信用卡分润", sys_value: "224114"},
			// {sys_name: "超级银行家分润", sys_value: "224116"}
		]
	...
	}else if($scope.oemType === "ZYFPAY"){			// 自由付
        $scope.activityTypess=[
            {sys_value: '7',sys_name:"欢乐返"},
            {sys_value: '8',sys_name:"欢乐返-循环送"},
            {sys_value: '11',sys_name:"注册返"}
	...
	}else if($scope.oemType === "YABPAY"){			// 宏博宏宇-安信宝
        $scope.activityTypess=[
            {sys_value: '7',sys_name:"欢乐返"},
            {sys_value: '8',sys_name:"欢乐返-循环送"},
            {sys_value: '11',sys_name:"注册返"}
	...
	}else if($scope.oemType === "REDEM"){
        $scope.subjectNoss = [
            {sys_name: "积分兑换账户", sys_value: "224120"}
        ];
	}else if($scope.oemType === "REDEMACTIVE"){
        $scope.subjectNoss = [
            {sys_name: "积分兑换(激活版)账户", sys_value: "224123"}
        ];
    }else if($scope.oemType === "PERAGENT"){
        $scope.subjectNoss = [
            {sys_name: "代理商分润", sys_value: "224105"},
            {sys_name: "活动补贴账户", sys_value: "224106"},
            {sys_name: "机具款项账户", sys_value: "224124"}
        ];
	}else if ($scope.oemType === 'ZHFPLUSPAY'){
        initData.ORDER_TYPE = initData.ORDER_TYPE.filter(function(item){
            // 屏蔽微创业订单"1" 欢乐送"2" 选项
        ...
        $scope.activityTypess = $scope.activityTypess.filter(function (item) {
        	// 只保留：欢乐返 7、欢乐返-循环送 8、注册返11 选项；
            if (item.sys_value == '7' || item.sys_value == '8' || item.sys_value == '11') {
                return true;
            }else{
                return false;
            }
	...
	$scope.whitelistTypes = initData.ROLL_TYPE;
	$scope.whitelistTypesAll = $scope.blacklistTypesAll;
	//预警方式
    ...
    $scope.cardType.push({text: '借记卡/贷记卡', value: 3});
	 ...
	 * 显示毫秒数 duration:
	 */
	$rootScope.notice=function(message,options){
		var option={ 
			message: '测试提示！',
			classes: 'alert-info', 
    ...
    $rootScope.orderNotice=function(message,options){
        var option={
            message: '测试提示！',
            classes: 'alert-info',
            templateUrl: "views/common/SurveyOrderNotify.html",
            position : "right",
            duration:300000
        };
        if (typeof options !== 'object'){
	...
	$scope.exportInfoClick=function (url,params) {
		...
		/*$http({
			...
			var fileName = response.headers("Content-Disposition").split(";")[1].split("filename=")[1];
			fileName = decodeURI(fileName) //防止中文乱码
...
function ngGridCtrl($scope) {
    ...
    $scope.ngOptions = {		
        ...
        enablePaging: true,
		 columnDefs: [
            {field: 'Name',displayName: '名称',width: 220,pinnable: false,sortable: false},
            {field: 'Age',displayName: '年龄',width: 220,pinnable: false,sortable: false},
            {field: 'Position',displayName: '位置',width: 220,pinnable: false,sortable: false,editable:true,
               cellTemplate: '<input style="width:98%;height:98%;" class="ui-widget input" ng-readonly="false" ng-model="row.entity[col.field]"/>'},
			{field: 'Status',displayName: '状态',width: 220,pinnable: false,sortable: false},
			{field: 'Date',displayName: '日期',width: 220,pinnable: false,sortable: false},
        ]    
	};
    $scope.ngOptions2 = {
        data: 'ngData',
...
function notifyCtrl($scope, notify) {
    ...
    $scope.inspiniaDemo1 = function(){
        notify({ message: 'Info - This is a Inspinia info notification', classes: 'alert-info'});
    }
 ...
 * timeEnd：结束时间回调函数
 */
verifyTimeFun=function(type,st,et,day,isVerifyTime,timeBegin,timeEnd){
	if (!isVerifyTime) {
		return;
	}
	var stime = new Date($(st).val()).getTime();
...
//    .controller('jstreeCtrl', jstreeCtrl)
	...
	}).filter("terminalFormatDropping",function(){
    return function(input,params){
        var showText = [];
        if(input){
            ...
            for (var i = 0; i < splits.length; i ++) {
                for(var key in params){
                    if(splits[i] && splits[i] == params[key].value){
                        showText.push(params[key].text);
                        break;
                    }
                }
            }
