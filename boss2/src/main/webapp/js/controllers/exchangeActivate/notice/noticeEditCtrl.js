 ...
 * 广告新增
 */
angular.module('inspinia').controller('exchangeActivateNoticeEditCtrl',function($scope,$http,i18nService,$state,$stateParams){
    ...
    $scope.getNotice=function () {
        $http.post("exchangeActivateNotice/getNotice","id="+$stateParams.id,
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                if(data.status){
    ...
    $scope.addNotice = function(){
        ...
        $http.post("exchangeActivateNotice/updateNotice",
