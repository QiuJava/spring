 ...
 * 产品类别列表
 */
angular.module('inspinia').controller('productTypeActivateQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            '<div class="lh30">'+
            '<a target="_blank" ui-sref="exchangeActivate.productTypeDetail({id:row.entity.id})">详情</a> ' +
            '<a ng-show="grid.appScope.hasPermit(\'productTypeActivate.editProductType\')" target="_blank" ui-sref="exchangeActivate.productTypeEdit({id:row.entity.id})" > | 修改</a> ' +
            '<a ng-show="grid.appScope.hasPermit(\'productTypeActivate.deleteProductType\')" ng-click="grid.appScope.deleteType(row.entity.id)""> | 删除</a> ' +
            '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("productTypeActivate/selectAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.deleteType = function(id){
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("productTypeActivate/deleteProductType","id="+id,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.deleteTypeBatch = function(){
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("productTypeActivate/deleteProductTypeBatch","ids="+ids,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.exportInfo = function () {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href = "productTypeActivate/importDetail?info=" + encodeURI(angular.toJson($scope.info));
                }
            });
    };
