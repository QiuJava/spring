 ...
 * 机构列表
 */
angular.module('inspinia',['angularFileUpload']).controller('orgManagementQueryCtrl',function($scope,$http,i18nService, FileUploader,SweetAlert,$document){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            '<div class="lh30">'+
            '<a target="_blank" ui-sref="exchange.orgManagementDetail({id:row.entity.id})">详情</a> ' +
            '<a ng-show="grid.appScope.hasPermit(\'orgManagement.updateOrgManagement\')" target="_blank" ui-sref="exchange.orgManagementEdit({id:row.entity.id})"> | 编辑</a> ' +
            '<a ng-show="row.entity.orgStatus==0&&grid.appScope.hasPermit(\'orgManagement.updateOrgStatus\')" ng-click="grid.appScope.updateOrgStatus(row.entity.id,row.entity.orgStatus)""> | 开启</a> ' +
            '<a ng-show="row.entity.orgStatus==1&&grid.appScope.hasPermit(\'orgManagement.updateOrgStatus\')" ng-click="grid.appScope.updateOrgStatus(row.entity.id,row.entity.orgStatus)"> | 关闭</a> ' +
            '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("orgManagement/selectAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $http.post("orgManagement/getaddOrgDefault",
        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
        .success(function(data){
            if(data.status){
    ...
    $scope.updateOrgStatus = function(id,orgStatus){
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("orgManagement/updateOrgStatus","id="+id+"&state="+state,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.addBank = function(){
        ...
        uploaderImg.onSuccessItem = function(fileItem,response, status, headers) {// 上传成功后的回调函数，在里面执行提交
            if (response.url != null) { // 回调参数url
                ...
                $http.post("orgManagement/addOrgManagement","info="+angular.toJson($scope.addinfo)+"&list="+angular.toJson($scope.listOrgDefault),
                    {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                    .success(function(data){
                        $scope.submitting = false;
                        if(data.status){
    ...
    $scope.exportInfo = function () {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href = "orgManagement/importDetail?info=" + encodeURI(angular.toJson($scope.info));
                }
            });
    };
