							...
							<div class="col-sm-2">
								<select class="form-control" name="status"> 
							         ...
							         <c:forEach var="adjustAccountStatus" items="${adjustAccountStatusList}">
										<option value="${adjustAccountStatus.sysValue}"
										<c:if test="${adjustAccountStatus.sysValue == params.subjectType}">selected="selected"</c:if>>
										${adjustAccountStatus.sysName}</option>
									</c:forEach>
								</select>                            
							 </div>   
							...
							<div class="col-sm-2">
								<select class="form-control" name="accountType"> 
							         ...
							         <c:forEach var="adjustAccountType" items="${adjustAccountTypeList}">
										<option value="${adjustAccountType.sysValue}"
										<c:if test="${adjustAccountType.sysValue == params.subjectType}">selected="selected"</c:if>>
   ...
   <script src="${ctx}/js/plugins/bootstrap-datepicker/locales/bootstrap-datepicker.zh-CN.min.js"></script>
	<script type="text/javascript">
		...
		function Detail(id) {   //单击修改链接的操作         
			...
			var id = model.id;
	        location.href='${ctx}/adjustAction/adjustRecordDetail.do?id='+id;
	    }
		function Modify(id) {   //单击修改链接的操作         
	        var model = $("#table_list_adjust_record").jqGrid('getRowData', id);
			var id = model.id;
	        location.href='${ctx}/adjustAction/adjustUpdate.do?id='+id;
		}
		
		function Shenhe(id) {   //单击修改链接的操作         
	        var model = $("#table_list_adjust_record").jqGrid('getRowData', id);
			var id = model.id;
	        location.href='${ctx}/adjustAction/adjustRecordExamine.do?id='+id;
		}
		...
		$(document).ready(function() {
		            ...
		            $("#table_list_adjust_record").jqGrid({
		            	url:"${ctx}/adjustAction/findAdjustRecord.do",
		                datatype: "json",
		                mtype: "POST",
		                height:"auto",
		                autowidth: true,
		                ...
		                rowNum: 10,
		                rowList: [10, 20],
		                colNames:['操作','调账ID','提交人', '提交时间', '审核人','审核时间','调账类型','状态','status'],
		                colModel: [
		                	{name:'Detail',index:'detail',width:240,align:"center",sortable:false,frozen:true},
		                    ...
		                    {name: 'accountType', index: 'account_type',width: 80, align: "left", sortable:false,formatter:customAccountTypeFmatter},
		                    {name: 'status', index: 'status',width: 80, align: "center", sortable:false,formatter:customAccountStatusFmatter},
		                    {name: 'status', index: 'status',width: 80, align: "center",hidden:"true"},
		                ],
		                    ...
		                    for(var i=0; i<ids.length; i++){
							    ...
							    if(getRow.applicant != username){
								    ...
								    <sec:authorize access="hasAuthority('adjustRecordListQuery:submitExamine')">
							    		detail += "&nbsp;&nbsp;<a href='javascript:void(0);' class='default-undetails' style='cursor:not-allowed' title='提交审核' >提交审核</a>";
							    	</sec:authorize>
