 ...
 * oem产品列表
 */
angular.module('inspinia',['infinity.angular-chosen']).controller('productOemQueryCtrl',function($scope,$http,i18nService,$stateParams,SweetAlert,$document,$timeout){
    ...
    $scope.getProductList=function (name) {
        $http.post("exchangeProduct/getProductList","name="+name+"&typeCode=-1",
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                if(data.status){
                    $scope.productList=[];
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            '<div class="lh30">'+
            '<a target="_blank" ui-sref="exchange.productOemDetail({id:row.entity.id,oemNo:row.entity.oemNo})">详情</a> ' +
            '<a ng-show="row.entity.shelve==2&&grid.appScope.hasPermit(\'exchangeOem.updateProductOem\')" target="_blank" ui-sref="exchange.productOemEdit({id:row.entity.id,oemNo:row.entity.oemNo})"> | 编辑</a> ' +
            '<a ng-show="row.entity.shelve==2&&grid.appScope.hasPermit(\'exchangeOem.updateProductOemShelve\')" ng-click="grid.appScope.productShelves(row.entity.id,row.entity.shelve)"> | 上架</a> ' +
            '<a ng-show="row.entity.shelve==1&&grid.appScope.hasPermit(\'exchangeOem.updateProductOemShelve\')" ng-click="grid.appScope.productShelves(row.entity.id,row.entity.shelve)"> | 下架</a> ' +
            '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("exchangeOem/selectProductOemList","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.productShelves=function (id,state) {
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("exchangeOem/updateProductOemShelve","id="+id+"&state="+shelves,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.productShelvesBatch=function (state) {
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("exchangeOem/updateProductOemShelveBatch","ids="+ids+"&state="+shelves,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
