...
public class TransInfoAction {
	...
	public Object getTotalNumAndTotalMoney(@RequestParam("info") String param) {
		...
		try {
			...
			CollectiveTransOrder totalInfo = transInfoService.queryNumAndMoney(cto);
	...
	public Object settleButch(@RequestBody String param) throws Exception {
		Map<String, Object> msg = new HashMap<String, Object>();
		try {
			List<CollectiveTransOrder> collectiveTransOrders = JSON.parseArray(param, CollectiveTransOrder.class);
			msg = transInfoService.settleButch(collectiveTransOrders);
		} catch (Exception e) {
			log.error("结算异常");
			System.out.println(e);
			e.printStackTrace();
	...
	public void exportInfo(@RequestParam("info") String info, HttpServletResponse response, HttpServletRequest request)
		...
		List<CollectiveTransOrder> list = transInfoService.importAllInfo(tis);
...
//					if ("0".equals(collectiveTransOrder.getRecommendedSource())) {
//						recommendedSource = "正常注册";
//					} else if ("1".equals(collectiveTransOrder.getRecommendedSource())) {
//						recommendedSource = "微创业";
//					} else if ("2".equals(collectiveTransOrder.getRecommendedSource())) {
//						recommendedSource = "代理商分享";
//					} else if ("3".equals(collectiveTransOrder.getRecommendedSource())) {
//						recommendedSource = "超级盟主";
//					}
				}
				// 收款类型
				...
				maps.put("accountNo",null == collectiveTransOrder.getAccountNo() ? "" : collectiveTransOrder.getAccountNo());
		...
		String[] cols = new String[] { "id", "orderNo", "groupCode", "settlementMethod", "merchantName",
				"merchantNo","merchantType","recommendedSource","acqReferenceNo","unionpayMerNo","payMethod", "orderType","zxRate", "serviceType", "cardType",
		...
		String[] colsName = new String[] { "交易流水", "订单号", "集群编号","结算周期", "商户简称",
				"商户编号","商户类型","推广来源","参考号", "银联报备商户编号", "交易方式", "订单类型","是否自选行业收费", "收款类型", "卡种",
				"交易卡号","卡类型(I:芯片卡, CI:非接芯片卡, S:磁条卡)","金额（元）","保费","商户费率", "交易手续费（元）", "抵扣交易手续费（元）","实际交易手续费（元）",
				"出款金额（元）", "到账金额（元）", "出款手续费（元）","实际出款手续费（元）","自选商户手续费（元）",
				"抵扣自选商户手续费（元）","实际自选商户手续费（元）", "云闪付费率（%）", "云闪付手续费（元）", "出款通道","交易状态","响应码","冻结状态", "结算状态",
...
//	@SystemLog(description = "代付订单导出", operCode = "trade.tradeSettleExport")
	public void exportShareSettleInfo(@RequestParam("info") String info, HttpServletResponse response, HttpServletRequest request)
		...
		List<ShareSettleInfo> list = transInfoService.exportShareSettleInfo(tis);
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd_HHmmss");
		SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
