        ...
        public String selectAllList(final Map<String, Object> param){
            ...
            String str=new SQL() {{
                if(red.getSumState()==0){
                    ...
                    SELECT(" sum(red.amount) amount_count");
                }
                FROM("red_orders_receive red");
//                LEFT_OUTER_JOIN("user_info ui on ui.user_code = red.");
                if(red.getId()!=null){
                    WHERE(" red.id=#{red.id}");
                }
                ...
                if(StringUtils.isNotBlank(red.getBusType())){
                	...
                	WHERE(" red.bus_type in (0,2)");
                }
                if(red.getOrgId()!=null){
                    WHERE(" red.org_id=#{red.orgId}");
                }else{
                	WHERE(" red.org_id=#{red.entityId}");//当前登录代理商
                }
