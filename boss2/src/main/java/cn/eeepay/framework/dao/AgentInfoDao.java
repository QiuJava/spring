...
public interface AgentInfoDao {
    ...
    @Select("SELECT CONCAT((SELECT count(1) FROM agent_share_rule_task rt"
		...
		+ " s.service_name,s.service_type,s2.service_type as service_type2,bpd.bp_name"
		+ " FROM"
		+ " agent_share_rule a "
    ...
    @Insert("insert into agent_info(agent_no,agent_node,agent_name,agent_level,parent_id,one_level_id,is_oem,team_id,email,phone,cluster,invest,"
    		+ "agent_area,mobilephone,link_name,invest_amount,address,account_name,account_type,account_no,bank_name,cnaps_no,sale_name,creator,"
	...
	@Insert("insert into agent_activity (activity_type_no,agent_no,agent_node,cash_back_amount,tax_rate,repeat_register_amount,repeat_register_ratio," +
			...
			"SELECT activity_type_no,agent_no,agent_node,'0','0','0','0','0','0','0','0' from activity_hardware_type,agent_info " +
	...
	@Insert("insert into agent_activity (activity_type_no,agent_no,agent_node,cash_back_amount,tax_rate,repeat_register_amount,repeat_register_ratio," +
			"full_prize_amount,repeat_full_prize_amount,not_full_deduct_amount,repeat_not_full_deduct_amount) " +
			"SELECT aht.activity_type_no,ai.agent_no,ai.agent_node,'0','0','0','0','0','0','0','0' from activity_hardware_type aht,agent_info ai " +
			"LEFT JOIN agent_activity aa on ai.agent_no=aa.agent_no and aa.activity_type_no =#{activityTypeNo} " +
			"where ai.status='1' and ai.agent_level!='1' and ai.agent_no = #{agentNo} and aht.activity_type_no =#{activityTypeNo} and aa.agent_no is null ")
	...
	@Insert("insert into agent_activity (activity_type_no,agent_no,agent_node,cash_back_amount,tax_rate,repeat_register_amount,repeat_register_ratio," +
			...
			"aht.full_amount,aht.repeat_full_amount,aht.empty_amount,aht.repeat_empty_amount from activity_hardware_type aht,agent_info ai " +
			"LEFT JOIN agent_activity aa on ai.agent_no=aa.agent_no and aa.activity_type_no =#{activityTypeNo} " +
    ...
    @Select("select * from  user_info where (mobilephone=#{mobilephone} or email=#{email}) and team_id=#{teamId}")
    ...
    @Select("select `id`, `share_id`,`efficient_date`,`effective_status`, `check_status`, `profit_type`, `per_fix_income`, "
    		+ "`per_fix_inrate`, `safe_line`, `capping`, `share_profit_percent`, `ladder`, `cost_rate_type`, `per_fix_cost`, "
    		+ "`cost_rate`, `cost_capping`, `cost_safeline`, `ladder1_rate`, `ladder1_max`, `ladder2_rate`, `ladder2_max`, "
    		+ "`ladder3_rate`, `ladder3_max`, `ladder4_rate`, `ladder4_max` from agent_share_rule_task "
    		+ "where check_status=0 and share_id=#{id} order by efficient_date desc")
    @ResultType(AgentShareRule.class)
    ...
    @Select("select `id`, `share_id`,`efficient_date`,`effective_status`, `check_status`, `profit_type`, `per_fix_income`, "
    		...
    		+ "`cost_rate`, `cost_capping`, `cost_safeline`, `ladder1_rate`, `ladder1_max`, `ladder2_rate`, `ladder2_max`, "
    		+ "`ladder3_rate`, `ladder3_max`, `ladder4_rate`, `ladder4_max` from agent_share_rule_task "
    		+ "where share_id=#{id} order by efficient_date desc")
    @ResultType(AgentShareRule.class)
    ...
    public class SqlProvider{
    	...
    	public String getServiceRate(Map<String,Object> map){
    		...
    		String sql = new SQL(){{
    			SELECT_DISTINCT("ser.*,info.service_name,info.service_type,info2.service_type as service_type2,bpd.bp_id,bpd.bp_name,bpd.allow_individual_apply");
...
//				ORDER_BY("bp.bp_id");
		...
		public String insertHappyBackType(Map<String, List<ActivityHardwareType>> param){
			...
			stringBuilder.append("insert into agent_activity (activity_type_no,agent_no,agent_node,cash_back_amount,tax_rate,repeat_register_amount,repeat_register_ratio," +
					"full_prize_amount,repeat_full_prize_amount,not_full_deduct_amount,repeat_not_full_deduct_amount) values");
...
//			sb.append(")");
    	...
    	public String getProfits(Map<String,Object> map){
    		...
    		String sql = new SQL(){{
    			SELECT_DISTINCT("asr.*,si.service_name");
    			FROM("business_product_info bp ");
    		...
    		SQL sql = new SQL(){{
    			...
    			if(StringUtils.isNotBlank(item)){
    				if(StringUtils.isNumeric(item)){
    					param.put("item", item+"%");
    					WHERE("agent_no like #{item}");
    				}else{
    					param.put("item", item+"%");
    					WHERE("agent_name like #{item}");
    				}
