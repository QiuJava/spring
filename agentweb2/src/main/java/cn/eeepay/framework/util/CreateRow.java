...
public abstract class CreateRow<T> {
    ...
    public void configRowCellStyle(Font rowFont, CellStyle oddCellStyle, CellStyle evenCellStyle) {
        ...
        oddCellStyle.setFillForegroundColor(IndexedColors.GREY_40_PERCENT.getIndex());
    ...
    protected String trimToEmpty(Object obj) {
        if (obj == null){
            return "";
        }
    ...
    protected String trimToDateFormat(Object obj, SimpleDateFormat sdf) {
        if (obj == null){
            return "";
        }
        try {
            return sdf.format(new Date(Long.valueOf(obj.toString().trim())));
        }catch (Exception e){
            return "";
        }
    }
