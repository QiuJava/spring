	...
	public void exportInfo(HttpServletResponse response,HttpServletRequest request,@RequestParam("info") String param) throws Exception{
		...
		if(list==null||list.size()<1){
			...
			for (int i = 0; i < list.size(); i++) {
				...
				//规则指令
				String rulesInstruction = "";
				if(list.get(i).getRulesInstruction().equals(1)){
					rulesInstruction = "商户黑名单";
				}else if(list.get(i).getRulesInstruction().equals(2)){
					rulesInstruction = "身份证黑名单";
				}else if(list.get(i).getRulesInstruction().equals(3)){
					rulesInstruction = "银行卡黑名单";
				}else if(list.get(i).getRulesInstruction().equals(4)){
					rulesInstruction = "钱包出账黑名单";
				}else if(list.get(i).getRulesInstruction().equals(5)){
					rulesInstruction = "实名认证";
				}else if(list.get(i).getRulesInstruction().equals(6)){
					rulesInstruction = "商户号风险预警";
				}else if(list.get(i).getRulesInstruction().equals(7)){
					rulesInstruction = "银行卡号风险预警";
				}else if(list.get(i).getRulesInstruction().equals(8)){
					rulesInstruction = "商户进件控制";
				}
				...
				maps.put("rulesInstruction",rulesInstruction);
		  ...
		  String[] cols = new String[]{
				  ...
				  "rulesInstruction","handleStatus","handleResults","handleTime","handlePerson","handleRemark","rollStatus"};
		  String[] colsName = new String[]{
				  "序号","创建时间","商户编号/身份证号/银行卡号","商户名称","代理商编号","规则编号","规则引擎",
				  "规则指令","处理状态","处理结果","处理时间","处理人","处理备注","黑名单状态"};
		  double[] cellWidth = {3000,6000,6000,6000,6000,10000,6000,6000,6000,6000,10000,6000,6000,10000};
