 ...
 * 超级兑用户详情/编辑
 */
angular.module('inspinia').controller('exchangeActivateUserDetailCtrl',function($scope,$state,$http,$stateParams,i18nService){
    //数据源
    ...
    $scope.getUserDetail=function(){
        $http.post("exchangeActivateUser/getUserManagement","merchantNo="+$stateParams.merchantNo,
    ...
    $scope.getUserAccountDetail = function(){
        ...
        $scope.loadImg = true;
        $http({
            url:"exchangeActivateUser/getUserAccountDetail?pageNo=" + $scope.paginationOptions.pageNo + "&pageSize=" + $scope.paginationOptions.pageSize,
    ...
    $scope.submit = function () {
        ...
        var data={
            ...
            "mobileUsername":$scope.info.mobileUsername
        };
        $http.post("exchangeActivateUser/updateUserManagement","info="+angular.toJson(data),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting=false;
                if(data.status){
                    $scope.notice(data.msg);
                    $state.transitionTo('exchangeActivate.userManagement',null,{reload:true});
                }else{
                    $scope.notice(data.msg);
                }
            }).error(function(data){
    ...
    $scope.updateUserCard = function () {
        ...
        $scope.submitting=true;
        $http.post("exchangeActivateUser/updateSettlementCard","info="+angular.toJson($scope.newCard),
            {headers: {'Content-Type':'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting=false;
                if(data.status){
    ...
    $scope.getDataProcessing = function () {
        if($scope.dataSta){
            $http.get('exchangeActivateUser/getDataProcessing?merchantNo='+$stateParams.merchantNo)
                .success(function(data) {
                    if(data.status){
                        $scope.info.mobileUsername = data.user.mobileUsername;
                        $scope.info.idCardNo = data.user.idCardNo;
                        if(data.card!=null){
                            $scope.card.mobileNo = data.card.mobileNo;
                        }
                        $scope.dataSta=false;
                    }else{
                        $scope.notice(data.msg);
