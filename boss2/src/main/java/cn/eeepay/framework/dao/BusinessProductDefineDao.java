...
public interface BusinessProductDefineDao {
	...
	@Insert("INSERT INTO business_product_define(bp_id,bp_name,sale_starttime,"
			...
			+ "(#{product.bpId},#{product.bpName},#{product.saleStarttime},"
	...
	@Select("select b1.*,b2.bp_name ownBpName,b3.bp_name linkProductName,t.team_name teamName from business_product_define b1 left join business_product_define"
			+ " b2 on b1.own_bp_id=b2.bp_id left join business_product_define b3 on b3.bp_id=b1.link_product left join team_info t on b1.team_id=t.team_id where b1.bp_id=#{id}")
	@ResultType(BusinessProductDefine.class)
...
//		}
		...
		public String selectByParam(Map<String, Object> param) {
			...
			return new SQL() {
				{
					SELECT_DISTINCT("bpd.bp_id, bpd.bp_name, bpd.bp_type, bpd.is_oem,bpd.effective_status,ti.team_name,bpd.create_person,bpd.create_time");
