 ...
 * 超级兑用户详情/编辑
 */
angular.module('inspinia').controller('userManagementEditCtrl',function($scope,$state,$http,$stateParams,i18nService){
    //数据源
    ...
    $scope.getUserDetail=function(){
        $http.post("userManagement/getUserManagementEdit","merchantNo="+$stateParams.merchantNo,
    ...
    $scope.submit = function () {
        ...
        $http.post("userManagement/updateUserManagement","info="+angular.toJson(data),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting=false;
                if(data.status){
                    $scope.notice(data.msg);
                    $state.transitionTo('exchange.userManagement',null,{reload:true});
                }else{
                    $scope.notice(data.msg);
                }
            }).error(function(data){
