 ...
 * 购买鼓励金活动详情
 */
angular.module('inspinia',['uiSwitch']).controller('buyRewardCtrl',function(i18nService,$scope,$http,$state,$stateParams,$compile,$filter,SweetAlert,$location){
	//购买鼓励金详情
	...
	$scope.add = function () {
        $location.url('/func/couponActivity/buyRewardDetailEdit/'+$stateParams.actId+'/add');
    }
	...
	$scope.rechargeEntityGrid = {                  //配置表格
		    ...
		    columnDefs:[                        //表格数据
		       ...
		       {field: 'isstatus', displayName: '是否可购买',width:100,cellTemplate:
	            	'<span ng-show="grid.appScope.hasPermit(\'couponActivity.updateStatus\')"><switch class="switch switch-s" ng-true-value="1" ng-false-value="0" ng-model="row.entity.isstatus" ng-change="grid.appScope.openStatus_Shelves(row,\'status\')" /></span>'
					+'<span ng-show="!grid.appScope.hasPermit(\'couponActivity.updateStatus\')"> <span ng-show="row.entity.isstatus==1">开启</span><span ng-show="row.entity.isstatus==0">关闭</span></span>'

			   },
	            {field: 'isshelves', displayName: '是否下架',width:100,cellTemplate:
	            	'<span ng-show="grid.appScope.hasPermit(\'couponActivity.updateStatus\')" ><switch class="switch switch-s" ng-true-value="1" ng-false-value="0" ng-model="row.entity.isshelves" ng-change="grid.appScope.openStatus_Shelves(row,\'isshelves\')" /></span>'
					+'<span ng-show="!grid.appScope.hasPermit(\'couponActivity.updateStatus\')"> <span ng-show="row.entity.isshelves==1">开启</span><span ng-show="row.entity.isshelves==0">关闭</span></span>'
				},
		       { field: 'action',displayName:'操作',width:100,
			      	 cellTemplate:
			      		  '<span><a  class="lh30" ui-sref="func.buyRewardDetailEdit({id:row.entity.id,view:\'view\'})">详情</a><span>'
			      		 +'<span><a  class="lh30" ui-sref="func.buyRewardDetailEdit({id:row.entity.id,view:\'edit\'})"> | 修改</a></span>'
			      		,width:130
			       }
		    ]
		};
