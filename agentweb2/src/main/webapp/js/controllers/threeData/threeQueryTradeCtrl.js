 ...
 */
angular.module('inspinia').controller('threeQueryTradeCtrl',function($scope,$rootScope,$http,$state,$stateParams,$compile,$uibModal,$log,i18nService,SweetAlert){
	...
	$scope.info={teamEntryId:'',merTeamId:"-1",orderType:"",agentNo:$rootScope.entityId,orderNo:"",transStatus:"SUCCESS",merchantNo:"",mobilephone:"",bool:'1',settlementMethod:"",settleStatus:"",
			terType:"",payMethod:"",serviceType:"",accountNo:"",smoney:"",emoney:"",activityType:"",merTeamId:"-1",startCreateTime:"",endCreateTime:"",minTransAmount:"",maxTransAmount:"",
  ...
  //是否校验时间
	...
	// })
	//代理商
	$http.post("agentInfo/selectConfigInfo").success(function(msg){
	...
	$scope.clear=function(){
		$scope.teamEntryIdList = [{text:'全部',value:''}];
		$scope.info={teamEntryId:'',merTeamId:$scope.initTeamId,orderType:"",agentNo:$rootScope.entityId,orderNo:"",transStatus:"SUCCESS",agentName:$rootScope.entityId,merchantNo:"",mobilephone:"",bool:'1',settlementMethod:"",
				terType:"",payMethod:"",serviceType:"",accountNo:"",smoney:"",emoney:"",sdate:"",edate:"",activityType:"",startCreateTime:"",endCreateTime:"",minTransAmount:"",maxTransAmount:"", serviceType: ''};
    ...
    $scope.query=function(){
        ...
        if (new Date($scope.info.edate).getTime() - new Date($scope.info.sdate).getTime() > 93*24*3600*1000) {
    	...
    	$http({
			url: "transInfoAction/getThreeAllInfo?pageNo=" + $scope.paginationOptions.pageNo + "&pageSize=" + $scope.paginationOptions.pageSize,
			method: 'POST',
			data: $scope.info
		}).success(function (msg) {
            $scope.queryTrading = false;
  ...
  /*  $scope.mobileFilter = function (mobile) {
    ...
    $scope.columnDefs = [//表格数据
     					...
     					{ field: 'mobilephone',displayName:'商户手机号',width:180, filter: {
     				          value:function(cellValue) {
     				             return  cellValue.substr(0,3)+'*****'+cellValue.substr(8,11)
     				           }
     				         } 
     					},
	...
	$scope.exportInfo=function(){
		...
		function (isConfirm) {
			if (isConfirm) {
				...
				location.href="transInfoAction/exportTransInfo?info="+angular.toJson($scope.info);
			}
		});
	}
