 ...
 * 积分兑换订单
 */
angular.module('inspinia').controller('exchangeActivateOrderQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            '<div class="lh30">'+
            '<a target="_blank" ui-sref="exchangeActivate.exchangeOrderDetail({id:row.entity.id})">详情</a> ' +
            '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("exchangeActivateOrder/selectAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.exportInfo = function () {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href = "exchangeActivateOrder/importDetail?info=" + encodeURI(angular.toJson($scope.info));
                }
            });
    };
