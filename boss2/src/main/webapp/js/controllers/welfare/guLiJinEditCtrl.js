 ...
 * 购买鼓励金活动详情
 */
angular.module('inspinia',['uiSwitch']).controller('guLiJinEditEditCtrl',function(i18nService,$scope,$http,$state,$stateParams,
                                                                                $compile,$filter,SweetAlert,$location){

    //编辑券配置
    if("view" == $stateParams.view){
        $("#coupon_title").text("鼓励金活动福利优惠券详情");
        $scope.isView = true;
    }else{
        $scope.isView = false;
        $("#coupon_title").text("修改鼓励金活动福利优惠券");
    }
    if("add" != $stateParams.view){
        ...
        $scope.info={};
        //$scope.couponEntity = {integralScale:1,cancelVerificationCode:1,couponCode:6,activetiyCode:6,isshelves:0,isstatus:0};
        $scope.couponEntity = {integralScale:1,cancelVerificationCode:1,couponCode:actId,activetiyCode:actId,isshelves:0,isstatus:0,couponType:6};
        $("#totalAmount").val(null);
        $scope.isView = false;
        $("#coupon_title").text("新增鼓励金活动福利优惠券");
    }
  ...
  $scope.isNum = function(obj,attr){
    ...
    if(attr==='couponCount' || attr==='purchaseCount') {
      var couponCount = $scope.couponEntity.couponCount;
      var purchaseCount = $scope.couponEntity.purchaseCount;
      console.log(purchaseCount +"   "+purchaseCount);
      if ((purchaseCount-couponCount) >0 ) {
        $scope.notice("商户每天可赠送数不能大于每天发行数！");
        obj[attr] = null;
        return;
      }
    }
    ...
    $scope.back = function () {
       /* $location.url('/func/couponActivity/buyReward/'+$stateParams.id+'/edit');*/
        $location.url('/func/welfare/guLiJin/'+$scope.couponEntity.activetiyCode+'/edit');
    }
