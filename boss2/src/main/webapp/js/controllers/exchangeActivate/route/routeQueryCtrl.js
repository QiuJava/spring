 ...
 * 路由列表
 */
angular.module('inspinia',['infinity.angular-chosen','uiSwitch','angularFileUpload']).controller('routeQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document,$timeout,FileUploader){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            {field: 'status',displayName: '状态',width: 180,
                cellTemplate:
                '<span ng-show="grid.appScope.hasPermit(\'excActRoute.closeRoute\')"><switch class="switch switch-s" ng-model="row.entity.status" ng-true-value="1" ng-false-value="0" ng-change="grid.appScope.closeRoute(row.entity)" /></span>'
                +'<span ng-show="!grid.appScope.hasPermit(\'excActRoute.closeRoute\')"> <span ng-show="row.entity.status==1">开启</span><span ng-show="row.entity.status==0">关闭</span></span>'
            ...
            { field: 'id',displayName:'操作',width:200,pinnedRight:true, cellTemplate:
                '<div class="lh30">'+
                '<a target="_blank" ui-sref="exchangeActivate.routeDetail({id:row.entity.id})">详情</a> ' +
                '<a ng-show="grid.appScope.hasPermit(\'excActRoute.updateRoute\')&&row.entity.status==0" target="_blank" ui-sref="exchangeActivate.routeEdit({id:row.entity.id})"> | 修改</a> ' +
                '<a ng-show="grid.appScope.hasPermit(\'excActRoute.addGood\')" target="_blank" ui-sref="exchangeActivate.routeGoodAdd({channelNo:row.entity.channelNo})"> | 增加商品</a> ' +
                '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("excActRoute/selectAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.closeRoute=function(entity){
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("excActRoute/closeRoute","id="+entity.id+"&state="+sta,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    var uploader = $scope.uploader = new FileUploader({
        url: 'excActRoute/importDiscount',
        queueLimit: 1,   //文件个数
