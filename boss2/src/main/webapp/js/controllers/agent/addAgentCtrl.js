angular.module('inspinia',['angularFileUpload','infinity.angular-chosen']).controller("addAgentCtrl", function($scope, $http, $state, $stateParams,uiGridConstants,i18nService,FileUploader,$log,$uibModal,$timeout) {
	...
	$scope.agent={agentName:"",isOem:0,email:"",mobilephone:"",phone:"",linkName:"",teamId:"",saleName:"",invest:0,
			investAmount:0,agentArea:"",address:"",accountName:"",accountType:2,bankName:"",accountNo:"",cnapsNo:"",isApprove:0,countLevel:-1,
        agentShareLevel:"",agentOem:"",agentType:""
	};
			...
			.success(function(data, status, headers, config) {  
				...
				angular.forEach(data.rates, function(data,index){
					...
					if(data.allowIndividualApply==1){
						...
						$scope.shareData[$scope.shareData.length] = {serviceId:data.serviceId,serviceName:data.serviceName,cardType:data.cardType,holidaysMark:data.holidaysMark,
								profitType:'5',bpId:data.bpId,bpName:data.bpName,serviceType:data.serviceType,serviceType2:data.serviceType2,shareProfitPercent:'100'};
					}
				});
