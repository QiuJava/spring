 ...
 * 积分兑换核销详情/核销
 */
angular.module('inspinia').controller('exchangeAuditDetailCtrl',function($scope,$http,i18nService,$stateParams,$state,SweetAlert){
    ...
    $http.post("exchangeOrder/getAuditExchangeOrder","id="+$stateParams.id,
        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
        .success(function(data){
            if(data.status){
                $scope.info=data.order;
                $scope.receiveState=data.receiveState;
                $scope.result=data.writeOffHisList;
            }
        });
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    ...
                    $http.post("exchangeOrder/updateWriteOff","info="+angular.toJson($scope.addInfo),
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    ...
                    $scope.submitting = true;
                    $http.post("exchangeOrder/updateAgainWriteOff","info="+angular.toJson($scope.addInfo),
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
