 ...
 * 机构编辑/详情
 */
angular.module('inspinia',['angularFileUpload']).controller('orgManagementEditCtrl',function($scope,$http,i18nService,$stateParams,FileUploader,$state){
    i18nService.setCurrentLang('zh-cn');  //设置语言为中文

    $scope.addinfo={};
    $http.post("orgManagement/getOrgManagementDetail","id="+$stateParams.id,
    ...
    $scope.saveOrg=function () {
        $http.post("orgManagement/updateOrgManagement","info="+angular.toJson($scope.addinfoSub)+"&list="+angular.toJson($scope.listOrgDefault),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
                    $scope.notice(data.msg);
                    $state.transitionTo('exchange.orgManagement',null,{reload:true});
                }else{
                    $scope.notice(data.msg);
                }
            })
