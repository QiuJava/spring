...
<#if !userInfo.withdrawProcess && bankCard??>
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-sm-9">
				...
				<#if !userInfo.withdrawProcess>
				<div style="margin: 10px 2px;" class="row h4">
					<div class="col-sm-4">
						可提现金额：<span class="text-primary">${account.usableBalance} 元</span>	
					</div>
					<div class="col-sm-4">
				...
				<div>
					<#if userInfo.withdrawProcess>
						<div class="el-tip-info">
							<p class="text-info">您有提现申请正在处理中，请完这个提现申请完成后再次申请；<a href="javascript:;">撤销提现申请</a></p>
						</div>
					<#elseif bankCard??>
					<form id="editForm" class="form-horizontal el-form" action="/website/withdraw/apply" method="post">
					<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
						<div class="form-group">
							...
							<div class="col-sm-10">
								...
								<script type="text/javascript">
									document.write(SITE_BANK_TYPE_NAME_MAP[${bankCard.bankCode}])
								</script>
								</label>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-10">
								<label class="form-control">${bankCard.bankForkName}</label>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-sm-2">银行卡号</label>
							<div class="col-sm-10">
								<label class="form-control">${bankCard.cardNumber}</label>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-sm-2">户主姓名</label>
							<div class="col-sm-10">
								<label class="form-control">${bankCard.accountName}</label>
							</div>
						</div>
						<div class="form-group">
