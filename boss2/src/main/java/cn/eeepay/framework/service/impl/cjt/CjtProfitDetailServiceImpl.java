...
public class CjtProfitDetailServiceImpl implements CjtProfitDetailService {
    ...
    public void export(HttpServletResponse response, CjtProfitDetail baseInfo) {
        ...
        try {
            ...
            String fileName = "超级推收益明细"+sdf.format(new Date())+export.getFileSuffix(size);
            ...
            for(CjtProfitDetail item: list){
                ...
                map.put("profitRateStr", item.getProfitRateStr()!= null ? item.getProfitRateStr().toString(): "");
    ...
    @Override
    public Result rechargeBatch(List<String> orderNoList) {
        String accountUrl = sysDictService.getValueByKey("ACCOUNT_SERVICE_URL") + "/superPushPlusController/supPushMerProfitRecordAccount.do";
        int successNum = 0;
        int failNum = 0;
        for(String orderNo: orderNoList) {
            ...
            try {
                //入账
                rechargeStatus = recharge(orderNo, accountUrl);
            } catch (Exception e) {
                log.error("批量入账失败", e);
            }
            if(rechargeStatus) {
     ...
     * @return
     */
    private boolean recharge(String orderNo, String url) {
        ...
        baseInfo.setTransTypeCode(CjtProfitDetail.accountTransTypeCode);
        String returnMsg = ClientInterface.cjtRecharge(baseInfo, url);
