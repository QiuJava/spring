 ...
 * 核销渠道商品明细查询
 */
angular.module('inspinia',['infinity.angular-chosen','uiSwitch']).controller('excRouteGoodQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document,$timeout){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            {field: 'status',displayName: '状态',width: 180,cellTemplate:
                '<span ng-show="grid.appScope.hasPermit(\'excRouteGood.closeGood\')"><switch class="switch switch-s" ng-model="row.entity.status" ng-true-value="1" ng-false-value="0" ng-change="grid.appScope.closeGood(row.entity)" /></span>'
                +'<span ng-show="!grid.appScope.hasPermit(\'excRouteGood.closeGood\')"> <span ng-show="row.entity.status==1">开启</span><span ng-show="row.entity.status==0">关闭</span></span>'
            ...
            { field: 'id',displayName:'操作',width:180,pinnedRight:true, cellTemplate:
                '<div class="lh30">'+
                '<a target="_blank" ui-sref="exchange.routeGoodDetail({id:row.entity.id})">详情</a> ' +
                '<a target="_blank" ng-show="grid.appScope.hasPermit(\'excRouteGood.updateGood\')&&row.entity.status==0" ui-sref="exchange.routeGoodEdit({id:row.entity.id})"> | 修改</a> ' +
                '<a ng-show="grid.appScope.hasPermit(\'excRouteGood.deleteGood\')&&row.entity.status==0" ng-click="grid.appScope.deleteGood(row.entity.id)"> | 删除</a> ' +
                '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("excRouteGood/selectAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.deleteGood=function(id){
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("excRouteGood/deleteGood","id="+id,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.closeGood=function(entity){
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("excRouteGood/closeGood","id="+entity.id+"&state="+sta,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.closeGoodBatch=function(sta){
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("excRouteGood/closeGoodBatch","ids="+ids+"&state="+sta,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.exportInfo = function () {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href = "excRouteGood/importDetail?info=" + encodeURI(angular.toJson($scope.info));
                }
            });
    };
