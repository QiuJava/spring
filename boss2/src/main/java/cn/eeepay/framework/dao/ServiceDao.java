...
public interface ServiceDao {
	...
	@Select("SELECT service_id,service_name ,service_type,t_flag,link_service FROM service_info WHERE service_id=#{id}")
	@ResultType(ServiceInfo.class)
	...
	@Insert("insert into service_info(service_name,service_type,hardware_is,bank_card,exclusive,business,sale_starttime,sale_endtime,use_starttime,use_endtime,proxy,"
			+ "getcash_id,rate_card,rate_holidays,quota_holidays,quota_card,oem_id,remark,t_flag,cash_subject,fixed_rate,fixed_quota,trad_start,trad_end,link_service,t0_turn_t1,create_person,create_time)"
			+ " values(#{s.serviceName},#{s.serviceType},#{s.hardwareIs},#{s.bankCard},#{s.exclusive},#{s.business},#{s.saleStarttime},#{s.saleEndtime},"
			...
			+ ",#{s.fixedRate},#{s.fixedQuota},#{s.tradStart},#{s.tradEnd},#{s.linkService},#{s.t0TurnT1},#{s.createPerson},#{s.createTime})")
	...
	@Update("update service_info set service_name=#{info.serviceName}, service_type=#{info.serviceType}, hardware_is=#{info.hardwareIs}, bank_card=#{info.bankCard}, exclusive=#{info.exclusive},fixed_rate=#{info.fixedRate},fixed_quota=#{info.fixedQuota},"
			...
			+ "getcash_id=#{info.getcashId}, rate_card=#{info.rateCard}, rate_holidays=#{info.rateHolidays}, quota_holidays=#{info.quotaHolidays}, quota_card=#{info.quotaCard}, oem_id=#{info.oemId}, remark=#{info.remark},t0_turn_t1=#{info.t0TurnT1} where service_id = #{info.serviceId}")
...
//	ServiceRate queryMinServiceRate(ServiceRate rate);
	...
	@Select("SELECT smr.rate,smr.rate_type,si.service_name,bpd.bp_name\n" +
	...
	@Select("SELECT smr.single_num_amount,smr.rate_type,si.service_name,bpd.bp_name from business_product_info bpi\n" +
			"LEFT JOIN business_product_group bpg on bpg.bp_id = bpi.bp_id\n" +
			"LEFT JOIN business_product_group bpg2 on bpg2.group_no = bpg.group_no\n" +
			"LEFT JOIN business_product_info bpi2 on bpi2.bp_id = bpg2.bp_id\n" +
			"LEFT JOIN service_info si on si.service_id = bpi2.service_id\n" +
...
//		}
	...
	@Select("SELECT asr.*, si.service_name FROM agent_share_rule asr "
