...
<script type="text/javascript">
	$(function() {
		...
		$("input[name=borrowStatus]").change(function(){
			$("#currentPage").val(1);
			searchForm.submit();
		});
	});
...
<body>
	...
	<div class="container" style="min-height: 500px;">
		...
		<form action="/website/invest/pageQuery" id="searchForm" method="post">
		<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
			...
			<div style="margin: 20px 0px;">
				  ...
				  <label class="btn btn-default active">
				    <input type="radio" name="borrowStatus" value="-1" autocomplete="off" checked />&emsp;全部&emsp;
				  </label>
				  <label class="btn btn-default">
				    <input type="radio" name="borrowStatus" value="1" autocomplete="off" />&emsp;招标中&emsp;
				  </label>
				  <label class="btn btn-default">
				    <input type="radio" name="borrowStatus" value="7" autocomplete="off" />&emsp;已完成&emsp;
				  </label>
				</div>
			</div>
		</form>
