 ...
 * 广告新增
 */
angular.module('inspinia').controller('noticeEditCtrl',function($scope,$http,i18nService,$state,$stateParams){
        ...
        isRowSelectable: function(row){ // 选中行
            if($scope.oemNoList != null){
                for(var i=0;i<$scope.oemNoList.length;i++){
                    if(row.entity.oemNo==$scope.oemNoList[i]){
    ...
    $scope.getNotice=function () {
        $http.post("exchangeNotice/getNotice","id="+$stateParams.id,
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                if(data.status){
    ...
    $scope.addNotice = function(){
        ...
        $http.post("exchangeNotice/updateNotice",
            ...
            .success(function(data){
                ...
                if(data.status){
                    ...
                    $state.transitionTo('exchange.notice',null,{reload:true});
                }else{
                    $scope.notice(data.msg);
                }
            })
