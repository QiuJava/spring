 ...
 * 产品列表
 */
angular.module('inspinia').controller('productQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            '<div class="lh30">'+
            '<a target="_blank" ui-sref="exchange.productDetail({id:row.entity.id})">详情</a> ' +
            '<a ng-show="grid.appScope.hasPermit(\'exchangeProduct.updateExchangeProduct\')" target="_blank" ui-sref="exchange.productEdit({id:row.entity.id})"> | 编辑</a> ' +
            '<a ng-show="grid.appScope.hasPermit(\'exchangeProduct.deleteExchangeProduct\')" ng-click="grid.appScope.deleteProduct(row.entity.id)""> | 删除</a> ' +
            '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("exchangeProduct/selectAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.deleteProduct=function (id) {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("exchangeProduct/deleteExchangeProduct","id="+id,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.addBank = function(){
        ...
        $http.post("exchangeProduct/addExchangeProduct","info="+angular.toJson($scope.addinfo)+"&list="+angular.toJson($scope.listOrgDefault),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
    ...
    $scope.exportInfo = function () {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href = "exchangeProduct/importDetail?info=" + encodeURI(angular.toJson($scope.info));
                }
            });
    };
