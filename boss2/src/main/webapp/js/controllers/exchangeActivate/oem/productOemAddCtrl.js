 ...
 * oem产品新增
 */
angular.module('inspinia',['infinity.angular-chosen']).controller('exchangeActivateOemProductAddCtrl',function($scope,$http,i18nService,$state,$stateParams,$timeout){
    ...
    $scope.saveOemProduct=function () {
        ...
        $http.post("exchangeActivateOem/addProductOem","info="+angular.toJson($scope.addinfo),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
