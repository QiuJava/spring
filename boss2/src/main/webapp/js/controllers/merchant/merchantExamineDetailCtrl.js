		...
		$scope.merchantDetailed={
			...
			columnDefs:[
	           ...
	           {field:'id',displayName:'操作',pinnable:false,sortable:false,
	        	   cellTemplate:
					   '<label ng-show="row.entity.mriId!=5" class="lh30"><input type="checkbox" ng-click="grid.appScope.rdt(1,COL_FIELD,$eval(\'row.entity.mriId\'))" name="a{{COL_FIELD}}" value="2"/>不通过'
					   +'<div ng-show="row.entity.mriId==37&&grid.appScope.editBtn" class="lh50" style="float: right;padding: 0px 5px;"><input class="btn" type="button" ng-click="grid.appScope.editMcc($eval(\'row.entity\'))" value="修改"  /></div>'
					   +'<div ng-show="row.entity.mriId==37&&grid.appScope.saveBtn" class="lh50" style="float: right;padding: 0px 5px;"><input class="btn" type="button" ng-click="grid.appScope.saveMcc($eval(\'row.entity\'))" value="保存"  /></div></label>'
	           }
			]
		};
