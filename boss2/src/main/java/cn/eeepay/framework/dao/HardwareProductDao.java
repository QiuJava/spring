...
public interface HardwareProductDao {
	...
	@Select("select hp.*,ti.team_name as orgName from hardware_product hp LEFT JOIN team_info ti ON hp.org_id=ti.team_id")
	 ...
	 * @return
	 */
	@Insert("INSERT INTO hardware_product (type_name,version_nu,facturer_code,manufacturer,model,org_id,secret_type,create_person,create_time,device_pn) "
			+ "VALUES (#{hardwareProduct.typeName},#{hardwareProduct.versionNu},#{hardwareProduct.facturerCode}," +
			"#{hardwareProduct.manufacturer},#{hardwareProduct.model},#{hardwareProduct.orgId},#{hardwareProduct.secretType},#{hardwareProduct.createPerson},#{hardwareProduct.createTime},#{hardwareProduct.devicePn})")

	int addHard(@Param("hardwareProduct") HardwareProduct hardwareProduct);

	
	
	@Insert("UPDATE hardware_product set type_name = #{hardwareProduct.typeName}, version_nu=#{hardwareProduct.versionNu},facturer_code=#{hardwareProduct.facturerCode},"
			+ "manufacturer=#{hardwareProduct.manufacturer},model=#{hardwareProduct.model},org_id=#{hardwareProduct.orgId}," +
