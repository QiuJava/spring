				...
				$(".return_money").click(function(){
					...
					if(useableAmount>=returnMoney){
						$.ajax({
							dataType:"json",
							type:"POST",
							url:"/repaymentSchedule/repay.do",
	...
	<body>
		...
		<div class="container">
			<div class="row">
				...
				<!-- 功能页面 -->
				<div class="col-sm-9">
					<form action="/repaymentSchedule.do" name="searchForm" id="searchForm" class="form-inline" method="post">
						<input type="hidden" id="currentPage" name="currentPage" value="1" />
						<div class="form-group">
							<label>时间范围</label>
							<input type="text" class="form-control beginDate" name="beginDate" value="${(qo.beginDate?date)!''}"/>
						</div>
						<div class="form-group">
							<label></label>
							<input type="text" class="form-control endDate" name="endDate" value="${(qo.endDate?date)!''}"/>
						</div>
						<div class="form-group">
						    <label>状态</label>
					...
					<div class="panel panel-default">
						...
						<input type="hidden" id="useableAmount" value="${account.usableAmount?number}">
						<table class="table">
							<thead>
								<tr>
									<th>借款</th>
							...
							<tbody>
								<#list pageResult.content as data>
									<tr>
										<td><a href="/borrow_info.do?id=${data.borrowId}">${data.borrowTitle}</a></td>
								        ...
								        <td>
								        	<#if data.state=0>
								        		<a href="javascript:;" class="return_money" data-returnmoney="${data.totalAmount?number}" data-rid="${data.id}">立刻还款</a>
								        	<#else>
								        		${data.stateDisplay}
								        	</#if>
								        </td>
