...
public class CardBinsServiceImpl implements CardBinsService {
    ...
    @Override
    public CardBins getCardBinsByCardNo(String cardNo) {
        return cardBinsDao.getCardBinsByCardNo(cardNo);
    }
    ...
    @Override
    public Map<String, Object> importCardBins(MultipartFile file) throws Exception {
        ...
        for (int i = 1; i <= row_num; i++) {
            ...
            String state = CellUtil.getCellValue(row.getCell(1));//状态:0关闭,1打开
            String cardType = CellUtil.getCellValue(row.getCell(2));//卡种:0其他,1贷记卡,2借记卡
            String cardBank = CellUtil.getCellValue(row.getCell(3));//发卡行
            String cardStyle = CellUtil.getCellValue(row.getCell(4));//卡类型
            String currency = CellUtil.getCellValue(row.getCell(5));//交易币种:取数据字典值
            String remarks = CellUtil.getCellValue(row.getCell(6));//备注
            if(cardNo==null||"".equals(cardNo)){
                msg.put("status", false);
                msg.put("msg","导入失败,第"+(i+1)+"行,卡bin为空!");
                return msg;
            }
            CardBins oldCardBins=getCardBinsByCardNo(cardNo);
            if(oldCardBins!=null){
                msg.put("status", false);
    ...
    public void exportCardBins(List<CardBins> list,HttpServletResponse response) throws Exception {
        ...
        if(list.size()<1){
            ...
            for (CardBins or : list) {
                ...
                if(or.getCardType()!=null){
                    if(or.getCardType()==0){
                        maps.put("cardType","其他");
                    }else if(or.getCardType()==1){
                        maps.put("cardType","贷记卡");
                    }else if(or.getCardType()==2){
                        maps.put("cardType","借记卡");
                    }else{
                        maps.put("cardType",null);
                    }
                }else{
        ...
        String[] cols = new String[]{"id","cardNo","state","cardType","cardBank","cardStyle",
                "currency","remarks","createTime","createName"
        };

        String[] colsName = new String[]{"编号","卡bin","状态","卡种","发卡行","卡类型",
                "交易币种","备注","创建时间","创建人"
        };
