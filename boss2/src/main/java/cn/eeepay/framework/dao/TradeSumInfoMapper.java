...
public interface TradeSumInfoMapper {
	...
	public class SqlProvider {
		public String tradeSumByList(Map<String, Object> params) {
			...
			final String[] oemArray = (String[]) params.get("oemArray");
			String sql = new SQL() {
				{
					...
					if (agentList != null && agentList.size() > 0) {
						...
						if (size <= 1000) {
							sb.append(" a_i.agent_no in ( ");
							for (String agentNo : agentList) {
							...
							for (int i = 0; i < size; i += 1000) {
								...
								for (int j = i; j < i + 1000; j++) {
									...
									sb.append(agentList.get(j)).append(",");
								}
								sb.deleteCharAt(sb.length() - 1);
								sb.append(" ) ");
								sb.append(" OR ");
					...
					// �?属组�?
					if (oemArray != null && oemArray.length > 0) {
						StringBuilder builder = new StringBuilder();
						builder.append("m_info.team_id IN (");
						for (int i = 0; i < oemArray.length; i++) {
							builder.append(oemArray[i]).append(",");
							if (i == oemArray.length - 1) {
			...
			String sql = new SQL() {
				{
					...
					if (agentList != null && agentList.size() > 0) {
						...
						if (size <= 1000) {
							sb.append(" a_info.one_level_id in ( ");
							for (String agentNo : agentList) {
								sb.append(agentNo).append(",");
							}
							sb.deleteCharAt(sb.length() - 1);
			...
			return new SQL() {
				{
					...
					if (qo.getAgentNoList() != null && qo.getAgentNoList().size() > 0) {
						StringBuilder sb = new StringBuilder();
						int size = qo.getAgentNoList().size();
						sb.append(" ( ");
						...
						if (size <= 1000) {
							...
							for (int i = 0; i < size; i += 1000) {
								...
								for (int j = i; j < i + 1000; j++) {
									...
									sb.append(qo.getAgentNoList().get(j)).append(",");
								}
								sb.deleteCharAt(sb.length() - 1);
								sb.append(" ) ");
								sb.append(" OR ");
			...
			return new SQL() {
				{
					...
					String agentOem = qo.getAgentOem();
					if (StringUtils.hasLength(agentOem)) {
						WHERE(" t_s_info.team_id = #{qo.agentOem}");
					}
					if (StringUtils.hasLength(qo.getStartTime())) {
						qo.setStartTime(qo.getStartTime() + " 00:00:00");
					...
					if (qo.getAgentNoList() != null && qo.getAgentNoList().size() > 0) {
						...
						if (size <= 1000) {
							sb.append(" t_s_info.agent_no in ( ");
							for (String agentNo : qo.getAgentNoList()) {
								sb.append(agentNo).append(",");
							}
							sb.deleteCharAt(sb.length() - 1);
	...
	List<SumDo> tradeSumByList(@Param("agentList") List<String> agentList, @Param("startTime") String startTime,
			@Param("endTime") String endTime, @Param("oemArray") String[] oemArray);
