...
public interface CarOrderDao {
	...
	class SqlProvider{
        public String findCarOrder(Map<String, Object> param){
            ...
            SQL sql = new SQL();
            sql.SELECT("om.order_no,tvi.violation_type,tvi.score,tvi.violation_time,tvi.violation_city,tvi.car_num,om.receive_amount,om.total_bonus");
        ...
        private String whereSql(SQL sql,CarOrder order){
        	if(order != null){
                 ...
                 if(StringUtils.isNotBlank(order.getOpenProvince())){
                     sql.WHERE("ui.open_province = #{order.openProvince}");
                 }
                 
                 if(StringUtils.isNotBlank(order.getOpenCity())){
                     sql.WHERE("ui.open_city = #{order.openCity}");
                 }
                 
                 if(StringUtils.isNotBlank(order.getOpenRegion())){
