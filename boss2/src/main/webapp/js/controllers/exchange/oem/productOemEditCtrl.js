 ...
 * oem产品修改
 */
angular.module('inspinia',['infinity.angular-chosen']).controller('productOemEditCtrl',function($scope,$http,i18nService,$state,$stateParams,$timeout){
    ...
    $scope.productChange=function () {
        if($scope.addinfo.pId!=null&&$scope.addinfo.pId!=null){
            $http.post("exchangeProduct/getExchangeProduct","id="+$scope.addinfo.pId,
                {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                .success(function(data){
                    if(data.status){
    ...
    $http.post("exchangeOem/getProductOemDetail","id="+$stateParams.id,
        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
        .success(function(data){
    ...
    $scope.saveOemProduct=function () {
        ...
        $http.post("exchangeOem/updateProductOem","info="+angular.toJson($scope.addinfo),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
                    $scope.notice(data.msg);
                    $state.transitionTo('exchange.productOem',{oemNo:$stateParams.oemNo},{reload:true});
                }else{
                    $scope.notice(data.msg);
                }
            })
