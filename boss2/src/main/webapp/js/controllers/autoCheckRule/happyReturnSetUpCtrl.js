...
i18nService.setCurrentLang('zh-cn');
    ...
    $scope.gridOptions008={						//配置表格
        ...
        columnDefs:[							//表格数据
            { field: 'typeName',displayName:'设备类型',width:200},
    ...
    $scope.checkActivityCode2 = function(){
        ...
        $http.post("activity/queryByactivityTypeNoList","009").success(function (data) {
            if(data.status){
                for(var i=0; i<data.info.length; i++){
                    $scope.typeNos2.push({value:data.info[i].activityTypeNo,text:data.info[i].activityTypeName,transAmount:data.info[i].transAmount});
                }
            }
        })
    };
    ...
    $scope.hardWardAddModel = function(){
        $scope.allHard = $scope.termianlTypes;
        $scope.saveStatus=0
        $("#hardWardAddModel").modal("show");
        $("#hardId").removeAttr("disabled");
        $("#activityCode").removeAttr("disabled");
        $scope.gridApiHard.selection.clearSelectedRows();
    ...
    $scope.saveHardWare = function(){
        var isNum=/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/;
        if($scope.gridApiHard.selection.getSelectedRows()==0&&($scope.hardWare.hardId == undefined || $scope.hardWare.hardId == null || $scope.hardWare.hardId=="")){
        ...
        $scope.submitting = true;
        var data = {"hardWare" : $scope.hardWare,"hards" : $scope.gridApiHard.selection.getSelectedRows()};
