...
* 黑名单处理
* */
angular.module('inspinia',['angularFileUpload']).controller('blackHandleCtrl',function($scope,$rootScope,$state,$filter,$log,$http,$stateParams,$compile,$uibModal,i18nService,SweetAlert,FileUploader,$location){
        ...
        $http.post('riskHandle/handle', 'orderNo=' + orderNo + '&&replyFilesName=' +replyFilesName +"&&replyRemark="+replyRemark,
            ...
            function (data) {
                if (data.header.succeed) {
                    $scope.notice(data.header.errMsg);
