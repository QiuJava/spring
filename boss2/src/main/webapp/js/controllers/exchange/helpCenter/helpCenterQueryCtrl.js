 ...
 * 产品列表
 */
angular.module('inspinia').controller('helpCenterQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'id',displayName:'操作',width:180,pinnedRight:true, cellTemplate:
                '<div class="lh30">'+
                '<a target="_blank" ui-sref="exchange.helpCenterDetail({id:row.entity.id})">详情</a> ' +
                '<a ng-show="grid.appScope.hasPermit(\'exchangeHelpCenter.updateHelpCenterState\')&&row.entity.status==1" ng-click="grid.appScope.updateHelpCenterState(row.entity)""> | 启用</a> ' +
                '<a ng-show="grid.appScope.hasPermit(\'exchangeHelpCenter.updateHelpCenterState\')&&row.entity.status==2" ng-click="grid.appScope.updateHelpCenterState(row.entity)""> | 关闭</a> ' +
                '<a ng-show="grid.appScope.hasPermit(\'exchangeHelpCenter.updateHelpCenter\')&&row.entity.status==1" target="_blank" ui-sref="exchange.helpCenterEdit({id:row.entity.id})"> | 修改</a> ' +
                '<a ng-show="grid.appScope.hasPermit(\'exchangeHelpCenter.deleteHelpCenter\')&&row.entity.status==1" ng-click="grid.appScope.deleteHelpCenter(row.entity.id)""> | 删除</a> ' +
                '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("exchangeHelpCenter/selectAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.deleteHelpCenter=function(id){
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("exchangeHelpCenter/deleteHelpCenter","id="+id,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.updateHelpCenterState=function(entry){
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("exchangeHelpCenter/updateHelpCenterState","id="+entry.id+"&state="+sta,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
