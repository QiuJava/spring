...
public interface ZqMerchantInfoDao {
   ...
   // createtime>=DATE_SUB(NOW(),INTERVAL 5 MINUTE);
    ...
    @Select("select unionpay_mer_no from zq_merchant_info where merchant_no =#{merNo} and mbp_id=#{bpId} and channel_code='ZG_ZQ'")
    @ResultType(ZqMerchantInfo.class)
    ZqMerchantInfo selectByMerNoAndBpId(@Param("merNo") String  merNo,@Param("bpId") String  bpId);
}
