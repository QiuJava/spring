 ...
 * 购买鼓励金活动详情
 */
angular.module('inspinia',['uiSwitch']).controller('buyRewardEditCtrl',function(i18nService,$scope,$http,$state,$stateParams,
                                                                                $compile,$filter,SweetAlert,$location){

    //编辑券配置
    if("view" == $stateParams.view){
        $("#coupon_title").text("购买鼓励金配置详情");
        $scope.isView = true;
    }else{
        $scope.isView = false;
        $("#coupon_title").text("修改购买鼓励金配置");
    }
    if("add" != $stateParams.view){
        ...
        $scope.couponEntity = {integralScale:1,cancelVerificationCode:1,couponCode:actId,activetiyCode:actId,isshelves:0,isstatus:0};
        $("#totalAmount").val(null);
        $scope.isView = false;
        $("#coupon_title").text("新增购买鼓励金配置");
    }
  ...
  //奖励有效期天数格式
    ...
    $scope.back = function () {
       /* $location.url('/func/couponActivity/buyReward/'+$stateParams.id+'/edit');*/
        $location.url('/func/couponActivity/buyReward/'+$scope.couponEntity.activetiyCode+'/edit');
    }
