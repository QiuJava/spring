...
public interface CjtProfitRuleDao {
	...
	@Select("select * from cjt_profit_rule where profit_type = #{profitType} and status = '1'")
	@ResultType(CjtProfitRule.class)
	List<CjtProfitRule> selectProfitRuleByType(@Param("profitType") String profitType);
	...
	@Select("select * from cjt_profit_rule where profit_type = #{profitType} limit 1")
	@ResultType(CjtProfitRule.class)
	CjtProfitRule selectFirstProfitRuleByType(String posTrade);
