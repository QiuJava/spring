...
//		console.log($scope.joinHbTypes);
		if($scope.joinHbTypes.length == 0){
			return;
		}
		angular.forEach($scope.happyGridApi.grid.rows,function(row){
			for(var i=0; i<$scope.joinHbTypes.length; i++){
				if(row.entity.activityTypeNo == $scope.joinHbTypes[i]){
					row.isSelected = true;
					break;
...
//			{field: 'profitType',displayName: '分润方式',width: 300,cellTemplate:
//				'<div class="lh30" ng-show="row.entity.isOldShare">'+
//					'{{row.entity.profitType | formatDropping:grid.appScope.fr}}'+
//				'</div>'+
//				'<div ng-show="row.entity.isNewShare"><select class="input-sm form-control input-s-sm inline" '+
//					'ng-model="row.entity.profitType" ng-options="x.value as x.text for x in grid.appScope.fr"/>'+
//				'</div>'
//			},
...
//				'<div ng-show="row.entity.profitType==1" style="height:98%;">'+
//					'<input type="text" style="width:98%;line-height:98%;height:34px; margin:0 1%;" class="ui-widget input"   ng-model="row.entity[col.field]"/>'+
//				'</div>'+
//				'<div ng-show="row.entity.profitType==2" style="height:98%;">'+
//					'<input type="text" style="width:98%;line-height:98%;height:34px; margin:0 1%;" class="ui-widget input"   ng-model="row.entity[col.field]"/>'+
//				'</div>'+
//				'<div ng-show="row.entity.profitType==3" style="height:98%;">'+
//					'<input type="text" style="width:98%;line-height:98%;height:34px; margin:0 1%;" class="ui-widget input"   ng-model="row.entity[col.field]"/>'+
//				'</div>'+
//				'<div ng-show="row.entity.profitType==4" style="height:98%;">'+
//					'<input type="text" style="width:98%;line-height:98%;height:34px; margin:0 1%;" class="ui-widget input"   ng-model="row.entity[col.field]"/>'+
//				'</div>'+
//				'</div>'
//			
...
//			{field: 'bbb',displayName: '阶梯分润设置',width: 130,
...
//			{field: 'ladderRate',displayName: '阶梯分润比例',width: 300},
			{field: 'aaa',displayName: '操作',width: 130,pinnedLeft:true,cellTemplate:
        		...
        		'</div>'
//        		+'<div class="lh30" ng-show="row.entity.isNewShare&&grid.appScope.hasPermit(\'agent.updateShare\')">'+
//        			'<div ng-show="row.entity.profitType==6"'+
//        				'<a ng-click="grid.appScope.ladderFr(row.entity)" >阶梯分润</a>'+
//        			'</div>'+
//        		'</div>'
            }
		]
...
//	    		$scope.oldShareInfo = {profitType:1};
    ...
    $scope.submitData = function(){
    	...
    	/*angular.forEach($scope.bpData,function(data,index){
    		if(data.aaa == true && !data.disabled){
    			bpIds[j] = data.key1;
    			j++;
    		}
    	});*/
