...
public class MerchantBusinessProductAction {
    ...
    @Resource
    private TimeIntervalCheckService timeIntervalCheckService;// 时间跨度校验service
...
/*	@DataSource(Constants.DATA_SOURCE_SLAVE)
	...
	public String  selectAllInfo(){
		...
		merchantNos.add("258121000031829");
	...
	*/
    ...
    public Object selectByStatusParam(@RequestParam("info") String param,
        ...
        try {
            ...
            // try {
            // timeIntervalCheckService.timeIntervalCheck(selectParams.getsTime(),selectParams.geteTime());
            // }catch (DateException e){
            // maps.put("msg", e.getMsg());
            // maps.put("status", false);
            // return maps;
            // }
    ...
    public boolean linkProduct(Map<String, String> params) {
        try {
            final UserLoginInfo principal = (UserLoginInfo) SecurityContextHolder.getContext().getAuthentication()
                    .getPrincipal();
            ...
            // List<HardwareProduct> hdWare =
            // hardwareProductService.findHardWareBybpId(linkBpId.toString());
            // if (hdWare == null || (hdWare != null && hdWare.size() > 1)) {
            // log.info("=====>>关联的业务产品不开通,关取业务产品ID[{}]系统配置多个硬件种类", new
            // Object[]{linkBpId.toString()});
            // return false;
            // }

            // String hpId = hdWare.get(0).getHpId().toString();
            // Object pos_type = (String) hdWare.get(0).getPosType();
            // if (pos_type == null) {
            // pos_type = "";
            // } else {
            // pos_type = pos_type.toString();
            // }
            ...
            boolean falg = merchantInfoService.linkProduct(params);
    ...
    public Object examineRcored(@RequestBody String param) {
        ...
        // ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ 防并发处理,20170814,mys ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        String status = merchantBusinessProductService.queryExamineStatus(bpId, merchantInfo.getMerchantNo());

        // liuks 商户状态为已审核时，不走下面逻辑

        if ("4".equals(status)) {
            // 复审时商户状态不受限制
            if (!"isReexamine".equals(json.get("isReexamine"))) {
                jsonMap.put("result", false);
                jsonMap.put("msg", "商户已通过自动审件审核，请勿重复提交");
                return jsonMap;
            }
        }
        if ("0".equals(status) || "1".equals(status)) {
            jsonMap.put("result", false);
            jsonMap.put("msg", "该件状态不符合，不能审核");
            return jsonMap;
        }

        // ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑ 防并发处理,20170814,mys ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑

        List<MerchantRequireItem> mri = JSON.parseArray(json.getJSONArray("listMri").toJSONString(),
                MerchantRequireItem.class);// 文字
        List<MerchantRequireItem> mris = JSON.parseArray(json.getJSONArray("listMris").toJSONString(),
                MerchantRequireItem.class);// 图片
        ...
        try {
            ...
            for (int i = 0; i < mri.size(); i++) {
                ...
                if (val == 1 || val == 3) {
                    isResult = true;
                    // 修改进件明细的状态
                    merchantRequireItemService.updateBymriId(mri.get(i).getId(), "1");
                ...
                if (mri.get(i).getMriId().equals("3")) {
                    ...
                    mci.setLogAccountNo(mri.get(i).getLogContent());
                }
                ...
                if (mri.get(i).getMriId().equals("7")) {
                    ...
                    if (str.length <= 3) {
                        throw new RuntimeException("经营地址格式不正确");
                    }
                ...
                if (mri.get(i).getMriId().equals("15")) {
                    ...
                    String[] str = content.split("-");
                    if (str.length < 3) {
                        throw new RuntimeException("开户行地址格式不正确");
                    }
                ...
                if (val == 4 || val == 2) {
                    if ("不通过".equals(mri.get(i).getaStatus())) {
                        ...
                        if (StringUtils.isNotBlank(teamName)) {
                            Sms.sendMsg(merchantInfo.getMobilephone(), "感谢您使用" + teamName.trim() + "，您提交的商户"
                                    + merchantInfo.getMerchantName() + "信息审核失败，失败原因为" + opinion + "");
                        }
                        if (mri.get(i).getMriId().equals("3") || mri.get(i).getMriId().equals("2")
                                || mri.get(i).getMriId().equals("6")) {
                            merchantBusinessProductService.updateCheckNum(merchantInfo.getMobilephone(),
            ...
            // 商户审核记录和银行卡添加商户地址修改
            if ("isReexamine".equals(json.get("isReexamine")) && num == 1) {// 由复审退件入口退件（商户下所有进件退件，此处ExamineType为后一个方法提供依据）
                el.setExamineType(2);
            }
            int i = (int) merchantBusinessProductService.updateMerBusiOProInfo(num, el, merbpId, merchantInfo, mci,
                    bpId);
            if (i > 0) {
                if (val == 1 || val == 3) {// 审核成功自动关联是否有关联
                    String merchantNo = merchantInfo.getMerchantNo();

                    // 调用商户进件接口
                    String autoLinkBpId = "0";
                    String params = "merchantNo=" + merchantNo + "&bpId=" + bpId + "&operator="
                            + principal.getId().toString() + "&autoLinkBpId=" + autoLinkBpId;
                    ...
                    //校验商户是否有过复审退件再次进件
                    MerchantBusinessProduct mbpInfo = merchantBusinessProductService.selectByPrimaryKey(Long.valueOf(merbpId));
                    if ("3".equals(mbpInfo.getReexamineStatus())) {
                        ...
                        List<MerchantBusinessProduct> mbpList = merchantBusinessProductService.getByMer(merchantInfo.getMerchantNo());
                        for (MerchantBusinessProduct mbp : mbpList) {
                            merchantBusinessProductService.resetReexamineBymbpId(mbp.getId(), "0", null);
                        }
                    }
                    // 审核成功发送短信提醒（审核成功不再发送短信，由core统一来发）
                }

                sensorsService.checkUser(merchantInfo, bpId, principal.getUsername(), opinion, isResult);

                // 跳下一条
                if (val == 3 || val == 4) {
                    Page<MerchantBusinessProduct> page = new Page<MerchantBusinessProduct>();
                    SelectParams ss = new SelectParams();
                    ss.setAuditorId(auditorId);
                    List<MerchantBusinessProduct> list = merchantBusinessProductService.selectByStatusParam(page, ss);
                    if (list.size() < 1) {
                        List<MerchantBusinessProduct> list12 = merchantBusinessProductService
                                .selectAllByStatusInfo(page);
                        if (list12.size() < 1) {
    ...
    public Object getAccountInfo(@RequestParam("merNo") String merNo, @RequestParam("bpId") String bpId) {
        ...
        try {
            ...
            ZqMerchantInfo zqMer = zqMerInfoService.selectByMerNoAndBpId(merNo, bpId);
            if (zqMer != null) {
