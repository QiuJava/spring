 ...
 * 产品编辑、详情
 */
angular.module('inspinia').controller('productEditCtrl',function($scope,$http,i18nService,$stateParams,$state){
    i18nService.setCurrentLang('zh-cn');  //设置语言为中文
    $scope.addinfo={orgCode:"",typeCode:""};
    $http.post("exchangeProduct/getExchangeProduct","id="+$stateParams.id,
    ...
    $scope.saveProduct=function () {
        ...
        $http.post("exchangeProduct/updateExchangeProduct","info="+angular.toJson($scope.addinfo),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
                    $scope.notice(data.msg);
                    $state.transitionTo('exchange.product',null,{reload:true});
                }else{
                    $scope.notice(data.msg);
                }
            })
