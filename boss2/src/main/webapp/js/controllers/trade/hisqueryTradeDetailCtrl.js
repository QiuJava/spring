 ...
 * 交易查询详情
 */
angular.module('inspinia').controller('hisqueryTradeDetailCtrl',function($scope,$http,$state,$stateParams,$compile,$uibModal,$log,i18nService){
...
//	$scope.settleStatusaa=[{text:'未结算',value:'0'},{text:'未结算',value:''},{text:'已结算',value:'1'},{text:'结算中',value:'2'},{text:'结算失败',value:'3'}]
	...
	if($stateParams.val=="0"){
	 ...
	 $http.get('transInfoHistoryAction/queryInfoDetail?ids='+$stateParams.id)
...
/*	 $scope.solutionModalOk=function(){
		//确认操作
     	...
     	$http.post("transInfoHistoryAction/tradeUnfreezeInfo",angular.toJson(data))
		.success(function(msg){
	       	  	if(msg.bols){
	       	  		$scope.notice(msg.msg);
	       	  		$state.go('histrade.tradeQuery');
	       	  	}else{
	       	  		$scope.notice(msg.msg);
	       	  	}
		 })
