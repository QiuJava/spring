...
public class MerchantBusinessProductServiceImpl implements MerchantBusinessProductService {
	...
	public Map<String, Object> updateByItemAndMbpId(MerchantBusinessProduct merchantBusinessProduct,
		...
		if (!misd.getMobilephone().equals(merchantInfo.getMobilephone()) || !misd.getMerchantName().equals(merchantInfo.getMerchantName()) || !merchantInfo.getRemark().equals(misd.getRemark()) || !merchantInfo.getLawyer().equals(misd.getLawyer()) || !merchantInfo.getStatus().equals(misd.getStatus())) {
...
//		recommendedSource.put("3","超级盟主");
	...
	public Object updateMerBusiOProInfo(int num, ExaminationsLog el, String merbpId, MerchantInfo merchantInfo, MerchantCardInfo mci, String bpId) {
		...
		if (num == 1) {
			//审核失败
			Boolean isReexamine =false;
			Boolean isLog =false;
			el.setOpenStatus("2");
			if(el.getExamineType()==2){
				//复审退件,获取当前商户其余进件，全部退件
				examinationsLogService.insert(el);
				i = updateBymbpId(Long.valueOf(merbpId), "3", el.getOperator());
				if (i == 0) {
					throw new RuntimeException("商户业务产品审核失败");
				}
				...
				for (MerchantBusinessProduct mbp:mbpList) {
					...
					if(!merbpId.equals(otherMbpId.toString())){
				...
				//正常审件
				MerchantBusinessProduct mbproduct = selectByPrimaryKey(Long.valueOf(merbpId));
				if("3".equals(mbproduct.getReexamineStatus()) ){
					//被复审退件的商户
					isReexamine = true;
					el.setExamineType(2);
					examinationsLogService.insert(el);
				...
				if(isReexamine){//退件其余进件
					log.info("属风控复审退件，继续退其他进件...");
					List<MerchantBusinessProduct> mbpList= getByMer(merchantInfo.getMerchantNo());
					final UserLoginInfo principal = (UserLoginInfo) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
					for (MerchantBusinessProduct mbp:mbpList) {
						Long otherMbpId = mbp.getId();
			...
			el.setOpenStatus("1");
			//商户表信息merchant_id
			i = merchantInfoService.updateByPrimaryKey(merchantInfo);
			if (i == 0) {
				throw new RuntimeException("商户信息审核失败");
			}
			//商户银行卡信息merchant_id
			...
			if (merchantCardInfoService.selectByMertIdAndAccountNo(mci) == null) {
				...
				if (i == 0) {
					throw new RuntimeException("商户银行卡信息审核失败");
				}
			} else {
			...
			i = updateBymbpId(Long.valueOf(merbpId), "4", el.getOperator());

			if (i == 0) {
				throw new RuntimeException("商户业务产品信息审核失败");
			}
			routeGroupDao.insertRouteGroupByMerchant(merchantInfo.getMerchantNo());
			boolean isReexamine=false;
			//校验商户是否有过复审退件再次进件
			MerchantBusinessProduct mbpInfo = selectByPrimaryKey(Long.valueOf(merbpId));
            ...
            if(isReexamine){
                log.info("商户曾被复审退件，再次进件，审核成功，其余件全部成功");
                List<MerchantBusinessProduct> mbpList= getByMer(merchantInfo.getMerchantNo());
                for (MerchantBusinessProduct mbp:mbpList) {
                    Long otherMbpId = mbp.getId();
                    if(!merbpId.equals(otherMbpId.toString())){
