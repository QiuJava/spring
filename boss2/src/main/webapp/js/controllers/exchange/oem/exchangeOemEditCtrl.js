 ...
 * oem新增
 */
angular.module('inspinia',['angularFileUpload','infinity.angular-chosen']).controller('exchangeOemEditCtrl',function($scope,$http,i18nService,$state,FileUploader,$timeout,$stateParams){
    ...
    //获取默认字段
    $http.post("exchangeOem/getExchangeOem", "id="+$stateParams.id,
    ...
    $scope.savetype = function(){
        ...
        if($scope.fileList!=null&&$scope.fileList.length>0){
            for(var i=0;i<$scope.fileList.length;i++){
                if($scope.fileList[i].isNeed=="1"){
                    if($scope.fileListDefault[i].configValue==null){
        ...
        $http.post("exchangeOem/updateExchangeOem",
