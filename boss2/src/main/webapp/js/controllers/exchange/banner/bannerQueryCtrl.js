 ...
 * 产品列表
 */
angular.module('inspinia',['uiSwitch']).controller('bannerQueryCtrl',function($scope,$http,i18nService,SweetAlert,$document){
    ...
    $scope.userGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            {field: 'status',displayName: '状态',width: 180,
                cellTemplate:
                '<span ng-show="grid.appScope.hasPermit(\'exchangeBanner.closeBanner\')"><switch class="switch switch-s" ng-model="row.entity.status" ng-true-value="1" ng-false-value="0" ng-change="grid.appScope.closeBanner(row.entity)" /></span>'
                +'<span ng-show="!grid.appScope.hasPermit(\'exchangeBanner.closeBanner\')"> <span ng-show="row.entity.status==1">开启</span><span ng-show="row.entity.status==0">关闭</span></span>'

            },
            { field: 'id',displayName:'操作',width:180,pinnedRight:true, cellTemplate:
                '<div class="lh30">'+
                '<a target="_blank" ui-sref="exchange.bannerDetail({id:row.entity.id})">详情</a> ' +
                '<a ng-show="grid.appScope.hasPermit(\'exchangeBanner.updateBanner\')" target="_blank" ui-sref="exchange.bannerEdit({id:row.entity.id})"> | 修改</a> ' +
                '<a ng-show="grid.appScope.hasPermit(\'exchangeBanner.deleteBanner\')&&row.entity.status==0" ng-click="grid.appScope.deleteBanner(row.entity.id)""> | 删除</a> ' +
                '</div>'
            }
        ],
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http.post("exchangeBanner/selectAll","info="+angular.toJson($scope.info)+"&pageNo="+
    ...
    $scope.deleteBanner=function(id){
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("exchangeBanner/deleteBanner","id="+id,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
    ...
    $scope.closeBanner=function(entity){
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("exchangeBanner/closeBanner","id="+entity.id+"&state="+sta,
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(data){
                            if(data.status){
                                $scope.notice(data.msg);
