angular.module('inspinia').controller('merchantQueryCtrl',function($scope,$rootScope,$http,$state,$stateParams,$compile,$filter,i18nService, SweetAlert){
	...
	$scope.info={merTeamId:"-1",quickPayment: "", riskStatus:'',recommendedSource:"",mbpId:"",merchantNo:"",merchantExamineState:"",agentName:$rootScope.entityId,agentNode:0,productType:"-1",termianlType:"-1",mobilephone:"",activityType:"",
	...
	if (['YPOSPAY', 'ZYFPAY', 'YABPAY', 'YLSTCZB'].indexOf($scope.oemType) >= 0){
        ...
        $scope.recommendedSources=[{text:"全部",value:""},{text:"正常注册",value:0},{text:"超级推",value:1},{text:"代理商分享",value:2}];
	}
    ...
    // 当前登录代理商的业务范围
    $http.get('userAction/getAccessTeamId').success(function (data) {
        if(data.status){
            $scope.info.merTeamId = data.accessTeamId;
        }
    });*/
	...
	$scope.selectInfo=function(){
		...
		$http({
			url: 'merchantBusinessProduct/selectByParam.do?pageNo='+$scope.paginationOptions.pageNo+"&pageSize="+$scope.paginationOptions.pageSize,
			method: 'POST',
			data: $scope.info
	...
	$scope.exportInfo = function () {
        ...
        function (isConfirm) {
            if (isConfirm) {
				...
				$scope.exportInfoClick("merchantBusinessProduct/exportMerchantInfo",{"info":angular.toJson($scope.info)});
            }
        });
    };
	...
	$scope.selectMechant=function(){
		$http.post('merchantBusinessProduct/selectMechant.do',
	...
	$scope.clear=function(){
		$scope.info={merTeamId:$scope.initTeamId,quickPayment:'', riskStatus:'',recommendedSource:"",
            sTime:moment(new Date().getTime() - 182 * 24 * 3600 * 1000).format('YYYY-MM-DD'+' 00:00:00'),
            eTime:moment(new Date().getTime()).format('YYYY-MM-DD')+' 23:59:59',
			mbpId:"",merchantNo:"",merchantExamineState:"",agentName:$rootScope.entityId,agentNode:0,productType:"-1",termianlType:"-1",mobilephone:"",activityType:""};
	};
    isVerifyTime = true;
	...
	$scope.columnDefs = [                           //表格数据
               		        /* { field: 'id',displayName:'商户进件编号',width:120},
               		         ...
               		         { field: 'sn',displayName:'机具编号',width:110},
