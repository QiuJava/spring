...
<script type="text/javascript">
	$(function() {
		...
		$("input[name=borrowState]").change(function(){
			$("#currentPage").val(1);
			searchForm.submit();
		});
	});
...
<body>
	...
	<div class="container" style="min-height: 500px;">
		...
		<form action="/invest/list.do" id="searchForm">
			<input type="hidden" name="currentPage" id="currentPage" value="1">
			<div style="margin: 20px 0px;">
					<span class="text-muted">标的状态</span><div style="margin-left: 30px" class="btn-group" data-toggle="buttons">
				  <label class="btn btn-default active">
				    <input type="radio" name="borrowState" value="-1" autocomplete="off" checked />&emsp;全部&emsp;
				  </label>
