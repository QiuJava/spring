...
angular.module('inspinia',['infinity.angular-chosen','angularFileUpload']).controller('lotteryOrderCtrl',function($scope,$http,i18nService,$document,SweetAlert,$timeout,FileUploader){
    ...
    setBeginTime=function(setTime){
        $scope.baseInfo.startTime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");

    }

    setEndTime=function(setTime){
        $scope.baseInfo.endTime = moment(setTime).format("YYYY-MM-DD HH:mm:ss");
    ...
    $scope.columnDefs = [
        ...
        {field: 'loanBankRate',displayName: '彩票机构给品牌扣率',width: 250,pinnable: false,sortable: false},
        {field: 'price',displayName: '品牌奖金',width: 250,pinnable: false,sortable: false},
        ...
        '<a class="lh30" rel="noopener noreferrer" target="_blank"  ui-sref="superBank.lotteryOrderDetail({orderNo:row.entity.orderNo})">详情</a>'}
    ];
