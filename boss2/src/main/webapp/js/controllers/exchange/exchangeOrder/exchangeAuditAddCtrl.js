 ...
 * 积分兑换订单新增
 */
angular.module('inspinia',['infinity.angular-chosen','angularFileUpload']).controller('exchangeAuditAddCtrl',function($scope,$http,i18nService,$stateParams,$state,$timeout,FileUploader){
    ...
    $scope.addExchangeOrder=function(){
        ...
        $http.post("exchangeOrder/addExchangeOrder","info="+angular.toJson($scope.addinfo),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                if(data.status){
                    $scope.notice(data.msg);
