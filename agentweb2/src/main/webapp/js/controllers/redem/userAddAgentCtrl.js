 ...
 */
angular.module('inspinia').controller('userAddAgentCtrl', function ($scope, $rootScope, $http, $state, $stateParams, $compile, $uibModal, $log, i18nService, SweetAlert) {
    ...
    $http({
        url: 'redemMerchant/queryAddMerchantInfo?merchantNo=' + $state.params.merchantNo,
        method: 'POST'
    ...
    $scope.submit = function () {
        ...
        $http({
            url: 'redemMerchant/userAddAgent',
            data: $scope.agent,
            method: 'POST'
        }).success(function (data) {
            $scope.submiting = false;
