 ...
 * 超级兑订单查询
 */
angular.module('inspinia', ['infinity.angular-chosen']).controller('superExchangeOrderCtrl',function($scope,$http,i18nService,$document,SweetAlert,$timeout){
    //数据源
    i18nService.setCurrentLang('zh-cn');
    $scope.paginationOptions=angular.copy($scope.paginationOptions);
    //订单类型:1代理授权；2信用卡申请 3收款 4信用卡还款 5贷款注册 6贷款批贷
    // $scope.statusList = [{text:"全部",value:""}];
    $scope.statusList = [{text:"全部",value:""},{text:"已创建",value:"1"},{text:"待支付",value:"2"},{text:"待审核",value:"3"},{text:"已完成",value:"5"},{text:"订单失败",value:"6"},{text:"已办理过",value:"7"},{text:"回收站",value:"9"}];//记账状态
    ...
    $scope.resetForm = function () {
        $scope.baseInfo = {orderType:"16",orderNo:"",status :"",bankName:"",orderName:"",
    ...
    $scope.columnDefs = [
        ...
        {field: 'bankName',displayName: '兑换机构别称',width: 150,pinnable: false,sortable: false},
        ...
        {field: 'price',displayName: '兑换价格',width: 150,pinnable: false,sortable: false},
        {field: 'payOrderNo',displayName: '关联订单号',width: 150,pinnable: false,sortable: false},
        ...
        {field: 'sharePhone',displayName: '贡献人手机号',width: 150,pinnable: false,sortable: false},
        ...
        '<a class="lh30" ng-show="grid.appScope.hasPermit(\'superBank.superExchangeOrderDetail\')" '
...
//		rowHeight:35,
   ...
   /* $scope.orgInfoList = [{orgId:-1,orgName:"全部"}];
    $scope.getBonusConfList = function () {
        $http({
            url:"superBank/getBonusConfList",
            method:"POST"
        }).success(function(msg){
            if(msg.status){
                $scope.orgInfoList = msg.data;
                $scope.orgInfoList.unshift({agencyAlias:"",agencyAlias:""});
            } else {
                $scope.notice("获取机构信息异常");
            }
        }).error(function(){
            $scope.notice("获取机构信息异常");
        })
    };
    $scope.getBonusConfList();*/
    ...
    $scope.exportInfo = function () {
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href="superBank/exportSuperExchangeOrder?baseInfo=" +encodeURI(encodeURI(angular.toJson($scope.baseInfo)));
                }
            });
    };
