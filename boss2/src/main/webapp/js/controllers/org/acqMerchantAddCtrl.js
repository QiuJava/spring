...
//	$scope.serviceTypes = [{text:"POS刷卡",value:1},{text:"扫码支付",value:2},{text:"快捷支付",value:3},{text:"账户提现",value:4}];
	$scope.acqServices=[];
	$scope.info={merchantServiceType:1,agentNos:"输入普通商户号后显示对应代理商",acqMerchantType:0,source:null};
...
// 	});
	...
	$scope.getOneAgentNo=function(){
		if(isNaN($scope.info.merchantNo)||$scope.info.merchantNo==""){
		 ...
		 $http.post("acqMerchantAction/selectOneAgentNo","merNo="+angular.toJson($scope.info.merchantNo),{headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
	 	 .success(function(msg){
	 			//响应成功
	 		 if(msg.bols){
