...
public class TradeSumJob extends ScheduleJob {
	...
	public void runTask(String runNo) {
		...
		// 拿到数据字典配置的所属组织
		String oemList = sysDictService.getValueByKey(THREE_INCOME_CALC_OEM);
		final String[] oemArray = oemList.split("-");
		...
		if ("1".equals(incomeHistoryCalcSwitch) && "1".equals(incomeCalcSwitch)) {
			taskExecutor.execute(new Runnable() {
				@Override
				public void run() {
					String startDateStr = sysDictService.getValueByKey(THREE_INCOME_HISTORY_CALC_START_DATE);
					String endDateStr = sysDictService.getValueByKey(THREE_INCOME_HISTORY_CALC_END_DATE);
		...
		taskExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				for (Date date : timeList) {
					...
					threeCalcByTeamId(oemArray, agentList, date, endTimeDate);
				}
		...
		Date endTimeDate = DateUtils.addDays(date, -Integer.valueOf(days));
		log.info("统计日期[{}]开始", defaultFormatDate);
		this.threeCalcByTeamId(oemArray, agentList, date, endTimeDate);
	...
	private void threeCalcByTeamId(String[] oemArray, List<String> list, Date date, Date endTimeDate) {
		...
		List<String> sumAgentList = tradeSumInfoMapper.findSumAgentNo(startTime, endTime, list);
		...
		List<SumDo> tradeSumList = tradeSumInfoMapper.tradeSumByList(agentList, startTime, endTime, oemArray);
		List<SumDo> merSumList = tradeSumInfoMapper.merSumByList(agentList, startTime, endTime, oemArray);
		List<SumDo> activateSumList = tradeSumInfoMapper.activateSumByList(agentList, startTime, endTime, oemArray);
		List<SumDo> machinesStockList = tradeSumInfoMapper.machinesStockByList(agentList, startTime, endTime, oemArray);
	...
	private void threeIncomeCalc(String agentNoStr, String startTime, String endTime, String teamId,
		...
		for (TransServiceIdDo transServiceIdDo : listSettleDo) {
			...
			if (rule == null) {
				...
				log.info("收益计算代理商[{}]分润规则-->服务id:[{}],代理商:[{}],卡类型:[{}],提现笔数:[{}]", agentNoStr, serviceId, agentNoStr,
						cardType, num);
			}
			String agentNode2 = transServiceIdDo.getAgentNode();
			...
			for (String noStr : agentNoList) {
				if (StringUtils.hasLength(noStr)) {
					if (chlidRule == null) {
						...
						if (chlidRule != null) {
							log.info("分润规则-->服务id:[{}],代理商:[{}],卡类型:[{}],提现笔数:[{}]", serviceId, noStr, cardType, num);
						}
					} else {
						break;
					}
			...
			for (String noStr : agentNoList) {
				if (StringUtils.hasLength(noStr)) {
					if (rate == null) {
						...
						if (rate != null) {
							log.info("管控费率-->服务id:[{}],代理商:[{}],卡类型:[{}],提现笔数:[{}]", serviceId, noStr, cardType, num);
						}
					} else {
						break;
					}
