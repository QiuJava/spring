 ...
 * banner新增
 */
angular.module('inspinia',['angularFileUpload']).controller('exchangeActivateBannerEditCtrl',function($scope,$http,i18nService,$state,FileUploader,$stateParams){
    ...
    $http.post("exchangeActivateBanner/getBanner",
        "id="+$stateParams.id,
        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
        .success(function(data){
            if(data.status){
    ...
    $scope.svaeBanner=function (url) {
        ...
        $http.post("exchangeActivateBanner/updateBanner",
            ...
            .success(function(data){
                ...
                if(data.status){
                    $scope.notice(data.msg);
                    $state.transitionTo('exchangeActivate.banner',null,{reload:true});
                }else{
                    $scope.notice(data.msg);
                }
            })
