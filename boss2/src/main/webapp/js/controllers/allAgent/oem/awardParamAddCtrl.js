 ...
 * oem新增
 */
angular.module('inspinia').controller('awardParamAddCtrl',function($scope,$http,i18nService,$state){
    ...
    $scope.clear=function(){
        $scope.addInfo={brandCode:"",outServiceId:"",oneAgentStatus:0,twoAgentStatus:0,userStatus:0};
        $scope.ladderInfo={};
        $scope.tradeResult=[];
        $scope.rewardResult=[];
        $scope.vipResult=[];
    ...
    $scope.tradeGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'id',displayName:'操作',width:120,pinnedRight:true, cellTemplate:
            '<div class="lh30">'+
    ...
    $scope.rewardGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'id',displayName:'操作',width:120,pinnedRight:true, cellTemplate:
            '<div class="lh30">'+
            '<a ng-click="grid.appScope.editData(2,row.entity)"">编辑</a> ' +
            '</div>'
            }
        ],
        onRegisterApi: function(gridApi) {
    ...
    $scope.vipGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            ...
            { field: 'id',displayName:'操作',width:120,pinnedRight:true, cellTemplate:
            '<div class="lh30">'+
            '<a ng-click="grid.appScope.editData(3,row.entity)"">编辑</a> ' +
            '</div>'
            }
        ],
        onRegisterApi: function(gridApi) {
    ...
    $scope.submitData=function () {
        ...
        if($scope.tradeResult==null||$scope.tradeResult.length<=0){
            $scope.notice("交易分润收益阶梯配置不能为空!");
            return false;
        }
        if($scope.rewardResult==null||$scope.rewardResult.length<=0){
            $scope.notice("荣耀奖金收益阶梯配置不能为空!");
            return false;
        }
        if($scope.vipResult==null||$scope.vipResult.length<=0){
            $scope.notice("VIP分润收益阶梯配置不能为空!");
            return false;
        }
        $scope.submitting = true;
        $http.post("awardParam/addAwardParam",
            "info="+angular.toJson($scope.addInfo)+
            "&tradeList="+angular.toJson($scope.tradeResult)+
            "&vipList="+angular.toJson($scope.vipResult)+
            "&crownList="+angular.toJson($scope.rewardResult),
            {headers: {'Content-Type':'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
