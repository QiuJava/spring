...
public class AgentInfoServiceImpl implements AgentInfoService{
	 ...
	 */
	@Override
	public String saveAgentInfo(JSONObject json) {
		String msg=null;
...
//				quotaList4.add(serviceDao.getServiceQuotaByQuota(quota));
		...
		if(happyBackTypes!=null&&happyBackTypes.size()>0){
			...
			/*value.append("('" + happyBackTypes.get(i).getActivityTypeNo());
			value.append("','" + agent.getAgentNo());
			value.append("','" + agent.getAgentNode());
			value.append("'," + happyBackTypes.get(i).getCashBackAmount());
			value.append("',1");
			value.append(i == happyBackTypes.size() - 1 ? ")" : "),");*/
			}


			int res = agentInfoDao.insertHappyBackType(happyBackTypes);
			if (res != happyBackTypes.size()) {
...
//			Map<String,Object> userInfo = agentInfoDao.getUserInfoById(userId);
		...
		//修改代理商参加的欢乐返子类型，只增不删-mays-2018.5.16-start
		List<ActivityHardwareType> happyBackTypes = JSONArray.parseArray(json.getString("happyBackTypes"), ActivityHardwareType.class);
		List<String> types = agentInfoDao.selectHbTypeByAgentNo(agent.getAgentNo());//查询已参加的子类型
		for (int i = 0; i < happyBackTypes.size(); i++) {//去掉已参加的子类型
			if (types.contains(happyBackTypes.get(i).getActivityTypeNo())) {
				happyBackTypes.remove(i--);
			}
		}
		if (happyBackTypes.size() > 0) {
			...
			/*if(!"".equals(tpyes)){
				...
				String agentNoStr[]=agent.getAgentNode().split("-");
				for(int i=2;i<agentNoStr.length;i++){
					if(agentNoStr.length>2){
						if(!StringUtil.isEmpty(agentNoStr[i])){
							agentInfoDao.insertHappyBackTypeByAgentLevel(agentNoStr[i],tpyes);
						}
					}
				}
			}*/
		...
		//修改代理商参加的欢乐返子类型，只增不删-mays-2018.5.15-end
