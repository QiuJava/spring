...
angular.module('inspinia', ['infinity.angular-chosen']).controller('queryTradeCtrl',function($scope,$http,$state,$stateParams,$compile,$uibModal,$timeout,$log,i18nService,SweetAlert,$document){
	...
	$scope.totalInfo = {};
	$scope.settleChangeStatus=[{text:"转T1结算",value:'4'},{text:"不结算",value:'5'}];//变更状态
	//机具类型
	$scope.termianlTypes=[];
	...
	$scope.clear=function(){
		...
		$scope.info={account:"",id:"",accountSerialNo:"",agentNo:"",settleStatus:"",orderNo:"",transStatus:"",freezeStatus:"",acqOrgId:"",merchantNo:"",mobilephone:"",bool:"",
				...
				orderType:"",zxStatus:"",recommendedSource:"",teamId:"",merchantType:"",authCodeStatus:""};
	}
	$scope.clear();
	...
	$scope.gridOptions={                           //配置表格
		      ...
		      columnDefs:[                           //表格数据
		          ...
		          { field: 'zxRate',displayName:'是否自选行业收费',width:150 },
				  ...
				  { field: 'deductionMerFee',displayName:'抵扣自选商户手续费（元）',cellFilter:"currency:''",width:180 },
				  { field: 'actualMerchantPrice',displayName:'实际自选商户手续费（元）',cellFilter:"currency:''",width:180 },
		        		   ...
		        		   '<div class="lh30" ng-switch on="$eval(\'row.entity.freezeStatus\')">'
						 		+'<div ng-switch-when=1>'
						 			+'<div ng-show="row.entity.isSettleMethod==1">'
								 		...
								 		+'<a ng-show="grid.appScope.hasPermit(\'trade.settleTransInfo\')" ng-click="grid.appScope.tradeSettle(row.entity.orderNo)"> | 结算</a> '
						 			+'</div>'
	...
	$scope.settleButch=function(){
		...
		angular.forEach($scope.list,function(item){
		 ...
		 SweetAlert.swal({
		        ...
		        function (isConfirm) {
		            if (isConfirm) {
		            	$http.post("transInfoAction/settleButch",angular.toJson($scope.settleList))
		 ...
		 if (!($scope.info.merchantNo || $scope.info.mobilephone || $scope.info.orderNo
			...
			if ((etime - stime) > (31 * 24 * 60 * 60 * 1000)) {
			 ...
			 SweetAlert.swal({
			        ...
			        function (isConfirm) {
			            if (isConfirm) {
					       	location.href="transInfoAction/exportTransInfo?info="+encodeURI(encodeURI(angular.toJson($scope.info)));
			            }
		        });
	 }
