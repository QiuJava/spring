 ...
 * 渠道商品新增
 */
angular.module('inspinia',['infinity.angular-chosen']).controller('routeGoodEditCtrl',function($scope,$http,i18nService,$state,$stateParams,$timeout){
    ...
    $scope.getRouteGood=function(){
        $http.post("excActRouteGood/getRouteGood","id="+$stateParams.id,
    ...
    $scope.commitInfo=function () {
        ...
        $http.post("excActRouteGood/updateGood","info="+angular.toJson($scope.addInfo),
            {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
                    $scope.notice(data.msg);
                    $state.transitionTo('exchangeActivate.routeGood',null,{reload:true});
                }else{
                    $scope.notice(data.msg);
                }
            })
